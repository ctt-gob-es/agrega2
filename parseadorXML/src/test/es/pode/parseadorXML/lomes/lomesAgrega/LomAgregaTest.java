	...
	public void testGeneral() {
		try {
			...
			if (url != null) {
				...
				Lom lom= dao.parsearLom(url.getFile());
				...
				try{
					...
					generalAgrega.insertarIdentificadorMEC("Catalogo MEC de prueba", "ID MEC de prueba");
					...
					ArrayList lKw2= generalAgrega.getPalabrasClaveAv("es");
...
//					lcAgrega.addContribucion("author", "autor del ODE", "2007-12-23", "Descripcion del autor");
					...
					EntidadAgrega sinVCard= lcAgrega.interpretaVCard("BEGIN:VCARD VERSION:3.0 FN: pepitorl ORG:");
//					sinVCard.setNombre(null);
//					sinVCard.setCorreo(null);
//					sinVCard.setOrganizacion(null);
					lcAgrega.escribirVCard(sinVCard);
		...
		try {
			...
			if (url != null) {
				...
				try{
					...
					EntidadAgrega sinVCard= mmdAgrega.interpretaVCard("BEGIN:VCARD VERSION:3.0 FN: EMAIL;TYPE=INTERNET: asdofhada END:VCARD");
//					sinVCard.setNombre(null);
//					sinVCard.setCorreo(null);
//					sinVCard.setOrganizacion(null);
					mmdAgrega.escribirVCard(sinVCard);
		...
		try {
			...
			if (url != null) {
				...
				try{
					...
					ArrayList orientaciones = eduAgrega.getOrientacionesDidacticas("es");
...
//					ArrayList dList= eduAgregaNuevo.getDestinatarios();
	...
	public void testRelacionesSeBasaEn() {
		try {
			URL url = this.getClass().getResource(
					"/" + properties.getProperty("test.lomes"));
			if (url != null) {
				...
				try{
					...
					relaciones= lomAgrega.getRelacionesSeBasaEnMEC();
					...
					String mensaje = "Error al obtener relaciones de LOM-ES: " +e;
				    logger.error(mensaje);
				}
			}
		}catch (ParseadorException e) {
	...
	public void testClassification() {
		try {
			URL url = this.getClass().getResource(
					"/" + properties.getProperty("test2.lomes"));
			if (url != null) {
...
//					ArrayList arbolesIds2= claAgrega.getIdsArbolesCurriculares("");
					...
					String mensaje = "Error al obtener elemento de LOM-ES: ";
...
//						claAgregaNuevo.removeTaxonPath(0);
	...
	public void testClassification02() throws Exception {
		try{
			URL url = this.getClass().getResource(
					"/" + properties.getProperty("test2.lomes"));
			if (url != null) {
				...
				List listaClassification= lomAgrega.getClassificationsAgrega();
				...
				for (Iterator iterator = listaClassification.iterator(); iterator.hasNext();) {
					...
					listaResultado = clasification.getIdsRutasTaxonomicas();
					logger.debug("tamaño " + listaResultado.size());
				}
				...
				claAgregaNuevo.setProposito("discipline");
				claAgregaNuevo.setFuente(0, "es", "Árbol curricular LOE 2006");
				claAgregaNuevo.setFuente(1, "es", "Nivel educativo LOM-ESv1.0");
				claAgregaNuevo.setFuente(2, "es", "fuente de prueba");
				
				claAgregaNuevo.addTaxon(0, "0.0.0", "Taxon de prueba", "es");
				claAgregaNuevo.addTaxon(1, "1.1.1", "Taxon de prueba", "es");
				claAgregaNuevo.addTaxon(2, "2.2.2", "Taxon de prueba", "es");
	
				assertNotNull(claAgregaNuevo.getIdsRutasTaxonomicas());
	
				ClassificationAgrega claAgregaVacio = new ClassificationAgrega(new Classification() );
				assertNull("ClassificationAgrega vacio, devería devolver un null",claAgregaVacio.getIdsRutasTaxonomicas());
			} else
				fail("Recurso de test "
						+ properties.getProperty("test.lomes")
						+ " no encontrado");
			...
			logger.error("Error en el test testClassification02: " , e);
		}
