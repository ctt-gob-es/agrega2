...
//De cada rama del tesauro, se devolverán todas los identificadores de sus taxones concatenados mediante el caracter '/'
  ...
  /**
   * Método público que permite obtener los ids de los taxones existentes. 
   * Cada Id está formado por la concatenación del nombre de la ruta (source) mas  
   * los ids de los taxones asociados separados por un caracter barra '/'.
   * 
   * @return ArrayList de String conteniendo ids de las rutas taxonómicas existentes en la Classification
   * @throws Exception
   */
  public ArrayList getIdsRutasTaxonomicas()
  throws Exception
  {
	  ...
	  try{
		  	...
		  	for (int i= 0; i< this.getTaxonPathsCount(); i++)
		  	{
		  		//obtengo el nombre del source para concatenarlo luego
				lsi= classification.getGroupClassificationClassification().getTaxonPath(i).getGroupTaxonPathTaxonPath().getSource().getGroupSourceSource().getLanguageStringItem();
				if(lsi!=null && lsi.length>0)
					source=lsi[0].getString().getContent() + separador;
				else
					source= "";
				
				// recorro los taxones de cada taxon path para obtener y
				// concatenar los id
				int nUltimoTaxon= this.getIndiceUltimoTaxon(i);
				for (int j= 0; j<= nUltimoTaxon; j++){
					//concateno los id separados con el caracter separador
					sb.append(this.getIdentificador(i, j)).append(separador);
				}
				//si tiene ids, le agregao el source y lo meto en la lista
  				if(sb.length()!=0 && !sb.toString().equals("")){
  					resultado.add(sb.insert(0, source  ).toString());
		  	...
		  	// si la lista está vacia devuelvo un null por coherencia con otros métodos similares
		  	if (resultado.size() == 0){
		  		resultado= null;
		  	}
	  ...
	  {
		  logger.error("Error en metodo getIdsRutasTaxonomicas", e);
		  resultado= null;
	  }
	  return resultado;
  }
