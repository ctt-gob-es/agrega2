...
//  metodo para escribir el identificador MEC en la primera posición
    public void setIdentificadorPrimeraPosicion(
...
//	   List idiomasAv=getIdiomas();
   ...
   //
   public ArrayList getPalabrasClaveAv (String idioma) throws java.lang.Exception
   {
	   ArrayList l= new ArrayList();
	   boolean encontrado = false;
   ...
   public void setEstructuraAv(String estructura) throws Exception{
	   try{
		   ...
		   estructuraProp =AgregaPropertiesImpl.getInstance().getProperty(AgregaProperties.ESQUEMA_LOMES); //"lomes-esv1.0";
		   if(estructura!=null){
...
//	   setNivelDeAgregacion(agregacion);
   ...
   try{
		   ...
		   lomes = AgregaPropertiesImpl.getInstance().getProperty(AgregaProperties.ESQUEMA_LOMES);//"lomes-esv1.0";
		   if(agregacion!=null){
			   AggregationLevel al= new AggregationLevel();
			   GroupAggregationLevelAggregationLevel gal= new GroupAggregationLevelAggregationLevel();
			   ComplexTypeAggregationLevelVocabSource ctalvs= new ComplexTypeAggregationLevelVocabSource();
   ...
   public void setIdentificadoresAv(ArrayList identificadores) throws Exception{
   	...
   	/**
   	 * Este metodo sirve para que no haya problemas con acentos, acentos codificados en HTML
   	 * o caracteres extraños a la hora de reconocer el ID del catalogo.
   	 * 
   	 * @param catalogoNodo: es el ID del catalogo unificado que usa el nodo para reconocer los IDs de los ODEs 
   	 * @param catalogoOde: es un ID de un catalogo que esta definido en el manifest de un ODE.
   	 * @return true si catalogoNodo es igual a catalogoOde sin tener en cuenta los caracteres no alfanumericos
   	 * ni las tildes expresadas en codigo HTML
   	 */
   	private boolean esIdCatalogoUnificado (String catalogoNodo, String catalogoOde) {
		
		String patron=catalogoNodo;
		String cadena=catalogoOde;
		
		cadena = cadena.replace("&aacute;","").replace("&Aacute;","").replace("&aecute;","").replace("&Eacute;","").replace("&iacute;","").replace("&Iacute;","").replace("&oacute;","").replace("&Oacute;","").replace("&uacute;","").replace("&Uacute;","").replace("&ntilde;","").replace("&Ntilde;","");
		//Eliminamos caracteres no alfanumericos; ñ, espacios, puntuacion, acentos, dieresis, etc.
		cadena = cadena.replaceAll("\\W", "");
		
		patron = patron.replace("&aacute;",".*").replace("&Aacute;",".*").replace("&aecute;",".*").replace("&Eacute;",".*").replace("&iacute;",".*").replace("&Iacute;",".*").replace("&oacute;",".*").replace("&Oacute;",".*").replace("&uacute;",".*").replace("&Uacute;",".*").replace("&ntilde;",".*").replace("&Ntilde;",".*");
	...
	// metodo para escribir el identificador MEC y nos devuelve el MEC que ha encontrado o si no "";
   	public String insertarIdentificadorMEC(java.lang.String cat, java.lang.String ent) throws java.lang.Exception{
		
   		String vuelta="";
		boolean identificadorMec=false;
		ArrayList arrayIdentificadores=getIdentificadoresAv();
		...
		for(int i=0;i<arrayIdentificadores.size() && !identificadorMec;i++){
			...
			if(esIdCatalogoUnificado(cat, identificador.getCatalogo())) {
			//if(identificador.getCatalogo().equals(cat)){
				...
				if(matcher.matches() || matcher2.matches()){
					vuelta=entrada;
					identificador.setCatalogo(cat);
					identificador.setEntrada(ent);
					arrayIdentificadores.set(i, identificador);
					identificadorMec=true;
				}
	 		}
	 	}
	 	if(!identificadorMec){
	 		IdentificadorAgrega nuevoIdentificador=new IdentificadorAgrega();
	 		nuevoIdentificador.setCatalogo(cat);
	 		nuevoIdentificador.setEntrada(ent);
	 		arrayIdentificadores.add(nuevoIdentificador);
	 	}
	 	setIdentificadoresAv(arrayIdentificadores);
		 return vuelta;   
	 }

	// metodo para obtener el identificador MEC;
	 public String obtenerIdentificadorFormatoMEC(
