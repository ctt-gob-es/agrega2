...
import es.pode.auditoria.negocio.servicios.NivelAgregacionVO;
...
public class MultiSearcher {
	 ...
	 * @throws Exception
	 */
	public static NivelAgregacionVO[] searchNivelAgregacion(ParametrosInformeFederadoVO parametros,List comunidades, String servicio, int maxWaitThread, int maxWaitPool) throws Exception{ 
		...
		NivelAgregacionVO[] resultado = new NivelAgregacionVO[new Integer(searched.get(0).toString()).intValue()];
			...
			if(!(documento == null) && documento.length > 0)
			{
				for(int j = 0; j < documento.length ; j++ ){
					if(!(documento[j] == null))
					{
						...
						informeNivelAgregacion.add(documento[j]);
					}
				}
		...
		resultado = (NivelAgregacionVO[])informeNivelAgregacion.toArray(new NivelAgregacionVO[0]);
			...
			if(!(documento == null) && documento.length > 0)
			{
				for(int j = 0; j < documento.length ; j++ ){
					if(!(documento[j] == null))
					{
						...
						informeCoberturaCurricular.add(documento[j]);
					}
				}
			...
			if(!(documento == null) && documento.length > 0)
			{
				for(int j = 0; j < documento.length ; j++ ){
					if(!(documento[j] == null))
					{
						...
						informeOdesIdioma.add(documento[j]);
					}
				}
			...
			if(!(documento == null) && documento.length > 0)
			{
				for(int j = 0; j < documento.length ; j++ ){
					if(!(documento[j] == null))
					{
						...
						informeOdesPublicados.add(documento[j]);
					}
				}
