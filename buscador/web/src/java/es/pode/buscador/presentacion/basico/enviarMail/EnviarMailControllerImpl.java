...
public class EnviarMailControllerImpl extends EnviarMailController
{
	...
	public final static String MENSAJE_ERROR_CAPTCHA="enviar.email.amigo.error.fallo.captcha";
    ...
    {    
    	...
    	if(!validCaptcha) {
    		errores.add(MENSAJE_ERROR_CAPTCHA);
    	...
    	if(form.getEnviarRemitente()!=null && form.getEnviarRemitente().booleanValue()){
    		if(logger.isDebugEnabled())logger.debug("Enviamos email para el remitente ["+form.getCorreoDesde()+"] con nombre ["+ form.getNombreDesde()+"]");
    	...
    	if(resultado.getResultado().equals("KO")){
            ...
            if(logger.isDebugEnabled())logger.debug("Se almacena en la BBDD el envio del ode con identificador en el idioma["+form.getIdioma()+"]");
            ...
            if (!registro.operacionEnviarCorreo(identificadorODE, form.getIdioma()))
            	...
            	if(logger.isDebugEnabled())logger.debug("El envio del correo se ha registrado correctamente en la base de datos");
    	}
    	if(logger.isDebugEnabled())logger.debug("Resultado del envío de correo --> " +resultado.getResultado());
    ...
    private void saveErrorMessage(HttpServletRequest request, List messages) throws Exception{
		if(logger.isDebugEnabled())logger.debug("Formulario incorrecto------------.");
		boolean encontrado;		
