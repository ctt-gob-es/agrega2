...
public class EliminarOdesControllerImpl extends EliminarOdesController{
    	...
    	try{
    		if (LdapUserDetailsUtils.esAdministrador() || LdapUserDetailsUtils.esPublicador()){
		        ...
		        if(form.getListaIds()!=null && !form.getListaIds().equals("")){
    		        ...
    		        String[] arrayListaIDs=(listIDs.split("&"));
				...
				logger.error("No tiene permisos para eliminar ODEs");
    ...
    public final void eliminarOdes(ActionMapping mapping, es.pode.buscador.presentacion.avanzado.eliminarOdes.EliminarOdesForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
    {
    	try{
    		...
    		{
    			if (form.getListaIds()!=null && !form.getListaIds().equals("")){
    				if(logger.isDebugEnabled())logger.debug("Es administrador");
					String[] identificadores = form.getListaIds().split("&");
					...
					for (int k=0;k<identificadores.length;k++){				
						...
						if(logger.isDebugEnabled())logger.debug("idODE: " + idODE + "   Titulo: " + titulo);
						String comentarios=EliminarOdesControllerImpl.getResource(MENSAJE_RESULTADO_CORRECTO, BuscarAvanzadoControllerImpl.APPLICATION_PROPERTIES, (Locale) request.getSession().getAttribute(ConstantesAgrega.DEFAULT_LOCALE));
						...
						{							
							if(logger.isDebugEnabled())logger.debug(" ODE eliminado correctamente");
							this.getSrvBuscarService().borrarHitCache();
						}else
							saveErrorMessage(request, "ERROR_ELIMINANDO_ODES");
					}
					try{
						if(logger.isDebugEnabled())logger.debug("Se va a eliminar la asociación de los ODEs eliminados a los tags sociales");
						this.getSrvTaggingServer().eliminarTagsDeOdes(identificadores);
					}catch(Exception e){
						logger.error("Error al intentar eliminar los ODEs seleccionados del tag social.",e);
						throw new Exception();
					}	
    			}else{
    				saveErrorMessage(request, "ERROR_ELIMINANDO_ODES");
