...
//			REALIZAMOS EL TRATAMIENTO DE LOS LITERALES PARA PODER INTRODUCIRLOS EN EL CÓDIGO HTML
			...
			String literalAplicacion=FormatoTag.getResource(literalApli, fichero, locale);
			String literalTexto=FormatoTag.getResource(literalText, fichero, locale);
			String literalImagen=FormatoTag.getResource(literalImag, fichero, locale);
			String literalAudio=FormatoTag.getResource(literalAud, fichero, locale);
			String literalVideo=FormatoTag.getResource(literalVid, fichero, locale);
...
//			RECUPERAMOS LOS TIPOS DE FORMATO			
	...
	public void pintarFormato (String[] tiposFormato,String literalAplicacion, String literalTexto, String literalImagen, String literalAudio, String literalVideo, JspWriter out) throws IOException{
		...
		for (int i=0;i<tiposFormato.length;i++){
			if (tiposFormato[i].trim().startsWith("application/") && !existe[0]){
				out.println("<img src='http://"+LdapUserDetailsUtils.getHost()+LdapUserDetailsUtils.getSubdominio()+"/static/Agrega2/img/icon_aplicacion.gif' alt="+literalAplicacion+" title="+literalAplicacion+" />" + "&nbsp;");
				existe[0]=true;
			}else if (tiposFormato[i].trim().startsWith("text/") && !existe[1]){
				out.println("<img src='http://"+LdapUserDetailsUtils.getHost()+LdapUserDetailsUtils.getSubdominio()+"/static/Agrega2/img/icon_texto.gif' alt="+literalTexto+" title="+literalTexto+" />" + "&nbsp;");
				existe[1]=true;
			}else if (tiposFormato[i].trim().startsWith("image/") && !existe[2]){
				out.println("<img src='http://"+LdapUserDetailsUtils.getHost()+LdapUserDetailsUtils.getSubdominio()+"/static/Agrega2/img/icon_imagen.gif' alt="+literalImagen+" title="+literalImagen+" />" + "&nbsp;");
				existe[2]=true;
			}else if (tiposFormato[i].trim().startsWith("audio/") && !existe[3]){
				out.println("<img src='http://"+LdapUserDetailsUtils.getHost()+LdapUserDetailsUtils.getSubdominio()+"/static/Agrega2/img/icon_audio.gif' alt="+literalAudio+" title="+literalAudio+" />" + "&nbsp;");
				existe[3]=true;
			}else if (tiposFormato[i].trim().startsWith("video/") && !existe[4]){
				out.println("<img src='http://"+LdapUserDetailsUtils.getHost()+LdapUserDetailsUtils.getSubdominio()+"/static/Agrega2/img/icon_video.gif' alt="+literalVideo+" title="+literalVideo+" />" + "&nbsp;");
				existe[4]=true;
			}
		}
    }
	...
	public void pintarFormato (String[] tiposFormato,String literalAplicacion, String literalTexto, String literalImagen, String literalAudio, String literalVideo, Boolean esVisualizable, JspWriter out) throws IOException{
    	
    	boolean[] existe= new boolean[6];
		for (int i=0;i<tiposFormato.length;i++){
			if (tiposFormato[i].trim().startsWith("application/") && !existe[0]){
				out.println("<img src='http://"+LdapUserDetailsUtils.getHost()+LdapUserDetailsUtils.getSubdominio()+"/static/Agrega2/img/icon_aplicacion.gif' alt="+literalAplicacion+" title="+literalAplicacion+" />" + "&nbsp;");
				existe[0]=true;
				...
				out.println("<img src='http://"+LdapUserDetailsUtils.getHost()+LdapUserDetailsUtils.getSubdominio()+"/static/Agrega2/img/icon_video.gif' alt="+literalVideo+" title="+literalVideo+" />" + "&nbsp;");
				existe[4]=true;
			} else if(!esVisualizable&&!existe[5]) {
				out.println("<img src='http://"+LdapUserDetailsUtils.getHost()+LdapUserDetailsUtils.getSubdominio()+"/static/Agrega2/img/icon_candado.gif' title='Restringido' />" + "&nbsp;");
				existe[5]=true;
			}
		}
    }
