			...
			try
			{
        		...
        		if((AgregaPropertiesImpl.getInstance().getProperty(AgregaProperties.INTEGRACION_LDAP)).equalsIgnoreCase("true"))
        		{
               		...
               		}else
            		{
            			...
            			if (listaPersonLdap.size()==0)
            				listaPersonLdap = autenticacionLdap.getUserLdapSecundario(usuario);
	            		...
	            		// 28062013 Se obtiene la propiedad del agrega.properties que indica si el usuario debe crearse activado o desactivado
	            		String sCrearUsuarioActivo= AgregaPropertiesImpl.getInstance().getProperty(AgregaProperties.ALTA_USUARIO_DESACTIVADO);
            			
	            		if(logger.isDebugEnabled())logger.debug("Valor de la propiedad 'altaUsuarioDesactivado': " + sCrearUsuarioActivo);	            		
	            		
	            		if (sCrearUsuarioActivo!=null && !sCrearUsuarioActivo.equals("") && sCrearUsuarioActivo.equals("true"))
	            		{
	            			if(logger.isDebugEnabled())logger.debug("Se va a crear el usuario desactivado.");
	            			usuarioVO.setFechaDesactivacion(Calendar.getInstance());
	            		}	            		
