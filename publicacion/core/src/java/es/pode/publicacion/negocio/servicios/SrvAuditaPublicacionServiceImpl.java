...
//    	Solo nos importa la fecha hasta como parametro de tiempo imprescindible.
    	...
    	{
    		logger.error("Error obteniendo el mumero de ODEs por estado. Fecha hasta["+fechaHasta==null?null:fechaHasta.toString()+"] vacia.");
    		throw new Exception("Error obteniendo el mumero de ODEs por estado. Fecha hasta["+fechaHasta==null?null:fechaHasta.toString()+"] vacia.");
    	}
   /*	
//    	Recuperamos los estados de la BD en orden del ciclo de vida de un ODE.
    	ArrayList listaEstadosOrdenada = new ArrayList();
...
*/
    	...
    	//Forma mas optima de hacer lo mismo que el codigo comentado arriba
    	Hashtable<String, String> mapaEstados = new Hashtable();	
    	try {
			for (int i = 0; i < SrvPublicacionServiceImpl.SECUENCIA_ESTADOS.length; i++) {
//				Si nos traemos el estadoVO, internacionaliza el texto del estado.
				EstadoVO estado = (EstadoVO)this.getEstadoDao().obtenEstadoPorNombre(this.getEstadoDao().TRANSFORM_ESTADOVO,SrvPublicacionServiceImpl.SECUENCIA_ESTADOS[i]);
				mapaEstados.put(estado.getClave(), estado.getNombre());
			}
		} catch (RuntimeException e) {
			logger.error("Imposible obtener estados de los ODEs.["+e.getCause()+"]");
    		throw new Exception("Imposible obtener estados de los ODEs.["+e.getCause()+"]");
		}
    	
		java.util.Collection transiciones = null;
		try {
			transiciones = this.getTransicionDao().obtenerTransicionesFinalesFechaAgrupandoEstado(DateManager.dateToCalendar(fechaHasta));
		} catch (Exception e){
			logger.error("Error al llamar al servicio de TransicionDao", e);
		}
		if (transiciones == null) return new ODEsEstadoVO[0];
		
    	ODEsEstadoVO[] resultadosEstados = new ODEsEstadoVO[transiciones.size()];
    	int i=0;
		for (Object object : transiciones) {
			Object[] transicion = (Object[])object;
			long cantidadEstado = ((Long)transicion[0]).longValue();
			String claveEstado = ((Estado)transicion[1]).getNombre();
			String nombreEstado = mapaEstados.get(claveEstado);
			ODEsEstadoVO resultado = new ODEsEstadoVO();
			resultado.setCantidad(cantidadEstado);
			resultado.setEstado(nombreEstado);
			resultadosEstados[i]=resultado;
			i++;
		}
        return resultadosEstados;		
...
//    	Creamos el criterio de busqueda
    	UltimosOdesPublicadosCriteria criterio = new UltimosOdesPublicadosCriteria();
    		criterio.setMaximumResultSize(numeroOdes);      	
