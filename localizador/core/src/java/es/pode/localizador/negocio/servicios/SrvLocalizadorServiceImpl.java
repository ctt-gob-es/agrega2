...
//				long consumoEspacio=calculaEspacio(new File(path));Si estamos generando el ODE el tamaño es 0
				...
				if(!file.mkdirs()) logger.debug("No se pudo crear");
	...
	protected LocalizadorVO handleCrearLocalizadorPublicado(String uuid, String mec) throws Exception {
		...
		try {
			Localizador localizador = this.getLocalizadorDao().buscarLocalizadorPorIdentificador(uuid);
...
//					de los que es capaz de absorber un sistema de ficheros.
					...
					this.getLocalizadorDao().create(this.getLocalizadorDao().fromLocalizadorVO(mecLocalizadorVO));
					...
					if (localizador.getMec().equals(mec)) { //si intenta crearlo con el mismo mec
						return this.getLocalizadorDao().toLocalizadorVO(localizador);
	...
	/**
	 * Este método permite crear un localizador para un ODE con su identificador MEC y el usuario dado. 
     * @param mec Identificador MEC del ODE.
     * @param usuario Usuario que crea el localizador.
     * @return se retorna el VO del localizador creado.
     * @throws Exception
     */
	protected LocalizadorVO handleCrearLocalizador(String mec, String usuario) throws Exception {
		if (mec == null || mec.equals(""))
		{
			CreaLocalizadorPublicadoException publicadoException = new CreaLocalizadorPublicadoException("Error intentando crear un localizador con mec vacio.");
			logger.warn(publicadoException);
			throw publicadoException;
		}
		if (usuario == null || usuario.equals(""))
		{
			CreaLocalizadorPublicadoException publicadoException = new CreaLocalizadorPublicadoException("Error intentando crear un localizador con usuario vacio.");
			logger.warn(publicadoException);
			throw publicadoException;
		}
		String dirBase = props.getProperty("publicacionPathBase");
...
//		 Se va a localizar por fecha
		String sFecha = this.getDirectorioFecha("");
		// tenemos los dos datos limpios. Creamos el localizador en la hubicación de los ODEs publicados.
//		Creamos un identificador MD5 a partir del id mec para que una carga de ODEs no meta en el directorio de una sola fecha más subdirectorios
//		de los que es capaz de absorber un sistema de ficheros.
		String md5 = EncriptacionUtiles.md5String(mec).substring(0, 2);
		String path= dirBase + sFecha + PATH_SEPARATOR+ md5+ PATH_SEPARATOR + mec;
		String url = urlBase+ sFecha + PATH_SEPARATOR + md5 + PATH_SEPARATOR +mec;
		if (logger.isInfoEnabled())logger.info("Cremamos localizador con mec["+mec+"] y usuario["+usuario+"] en path["+path+"] md5["+md5+"]");
		long consumoEspacio=calculaEspacio(new File(path));
		return (LocalizadorVO)this.getLocalizadorDao().create(this.getLocalizadorDao().TRANSFORM_LOCALIZADORVO,path, url,"",mec,usuario,consumoEspacio);
	}
	...
	/*
	 ...
	 private long calculaEspacio(File localizacion)
	 {
		 try{
			 ...
			 logger.error("Error al calcular el espacio del fichero '"+localizacion.getAbsolutePath()+"' "+e);
			 long vuelta=0;
			 return vuelta;
		 }
