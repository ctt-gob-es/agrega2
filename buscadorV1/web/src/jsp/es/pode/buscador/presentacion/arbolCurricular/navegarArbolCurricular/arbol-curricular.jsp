     ...
     <div class="plantilla_contenido">
			...
			<div class="caja_tabla" >
				...
				<div class="breadcrumb_curricular">
					...
					<form method="post" action="<html:rewrite action="/NavegarArbolCurricularCU/ArbolCurricularBuscarAvanzado.do?areaCurricularBusqueda=${ultimo.id}&tipoBusqueda=${form.tipoBusqueda}&tipoNavegacion=${form.tipoNavegacion}&busquedaArbol=true"/>" >
						<html:link styleClass="br_carpeta_abierta" action="/NavegarArbolCurricularCU/ArbolCurricularSeleccionarHijo.do?tipoBusqueda=${form.tipoBusqueda}&tipoNavegacion=${form.tipoNavegacion}">
       						<bean:message key="navegar.taxonomia.inicio"/><br/>
			      		</html:link>
			        ...
			        <c:choose>
	        			<c:when test="${taxonesvoact.esHoja}">
	        				<display:column class="new" autolink="true">
	        					...
	        					<c:if test="${taxonesvoact.numeroOdesAsociados!=0}">
			            			<strong><html:link styleClass="carpeta_cerrada" style="display:inline" action="/NavegarArbolCurricularCU/ArbolCurricularBuscarAvanzado.do?areaCurricularBusqueda=${taxonesvoact.identificador}&tipoBusqueda=${form.tipoBusqueda}&numMaxResultados=${taxonesvoact.numeroOdesAsociados}&tipoNavegacion=${form.tipoNavegacion}&busquedaArbol=true">
			        					${taxonesvoact.nombre} 
			        				</html:link></strong>
		        				</c:if>
	        			...
	        			<c:otherwise>
	        				<display:column  class="new" autolink="true">
	        					...
	        					<c:if test="${taxonesvoact.numeroOdesAsociados!=0}">
		        				<strong><html:link styleClass="carpeta_cerrada" style="display:inline" action="/NavegarArbolCurricularCU/ArbolCurricularSeleccionarHijo?areaCurricularBusqueda=${taxonesvoact.identificador}&tipoBusqueda=${form.tipoBusqueda}&numeroResultados=${taxonesvoact.numeroOdesAsociados}&tipoNavegacion=${form.tipoNavegacion}">
		        					${taxonesvoact.nombre} 
		        				</html:link></strong>
	        					</c:if>
	        					<em  style="display:inline;">(<c:out value="${taxonesvoact.numeroOdesAsociados}"/>
