...
<!-- CUERPO PRINCIPAL  -->
  ...
  <form method="post" id="taxonomiaAsociarForm" action="<html:rewrite action="/AreaCurricularCU/TaxonomiaVolcarTaxonomia"/>">
	...
	<h2><bean:message key="taxonomias.nombre"/>${form.taxNombre}</h2>
	...
	<!-- Inicio Botones  -->
	<!-- Inicio Botones  -->
	<input class="boton_125_de_2"  type="submit"  value="<bean:message key="taxonomias.boton.asociar"/>" />
	</form>
	<form method="post" action="<html:rewrite action="/BuscarAvanzadoCU/BuscarAvanzadoCU.do"/>">
		<input type="hidden" name="tipoLayoutBuscador" value="${form.tipoLayoutBuscador}"/>
	    <input class="boton_125_de_2_izq"  type="submit"  value="<bean:message key="taxonomias.cancelar.seleccion"/>"/>
	</form>
	<!-- Fin Botones  -->
	</div>
