...
//	        					odesRelacionados[j]=resultadosBusqueda[i];
	        ...
	        //Orientacion didáctica,palabras clave y tipo de recurso
	        	try{
	        	// 11042012 Añadimos la comprobación metadatos.getOrientacionDidactica().length>0 porque parece ser que en distrubuciones para Oracle
	        	//			la propiedad no es nula cuando está vacia y provoca un error al acceder a la posición 0 directamente
	        ...
	        // 22082013 Evolutivo Modificación de ODEs con versión
	        //			Comprobamos si el ODE lo creó el usuario logado para mostrar o no el botón de crear nueva versión.
	        form.setPermisoNuevaVersion(false);
	        
	        String[] idOdes = new String[1];
	        idOdes[0]=form.getIdentificadorODE();
	        
	        String[] usuariosCreacion = getSrvPublicacionService().obtenerUsuariosCreacionPublicadosIdentificadores(idOdes);
	        
	        // Si devuelve usuario de creación y es igual al logado es porque el ODE es suyo y se le permite versionarlo
	        if (usuariosCreacion!=null && usuariosCreacion.length>0 && LdapUserDetailsUtils.getUsuario().equals(usuariosCreacion[0]))
	        {	        
	        	form.setPermisoNuevaVersion(true);	        	
	        }	        
	        if(logger.isDebugEnabled())logger.debug("DetallarControllerImpl - buscarDetalleODE: Tiene permiso de versión :"+form.getPermisoNuevaVersion());
...
//		de la busqueda avanzada o basica dependiendo de a donde tengamos que ir.
	...
	//			 ODE federado, devuelvo DataHandler al empaquetador
				try {
					String identificadorPpal = sesion.getEmpIdOde();
					String identificadorSubmanif = form.getIdentificadorODE();
					if(sesion.getNodoOrigen()!=null && !sesion.getNodoOrigen().trim().equals("")){
					...
					form.setMostrarMensaje(false);
					getSrvGestorManifestService().guardarManifiestoControlandoBackups(identificadorPpal, false, false);
					ValidarLicenciasVO validarlicencias=getSrvGruposLicencias().cambiaLicenciaCompatibleRuta(identificadorPpal, identificadorSubmanif, LdapUserDetailsUtils.getUsuario(), LdapUserDetailsUtils.getIdioma());
			   		String mensaje="";
