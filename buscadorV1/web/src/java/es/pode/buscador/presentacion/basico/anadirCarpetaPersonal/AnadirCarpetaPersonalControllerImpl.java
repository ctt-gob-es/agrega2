...
public class AnadirCarpetaPersonalControllerImpl extends AnadirCarpetaPersonalController
{
	...
	private final static String MENSAJE_COMENTARIOS_NUEVA_VERSION = "comentarios.nueva.version.ode";
    ...
    public final void anadirODECarpetaPersonal(ActionMapping mapping, es.pode.buscador.presentacion.basico.anadirCarpetaPersonal.AnadirODECarpetaPersonalForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
    {
    	...
    	try{ 	
			...
			// 21082013 Verificamos si en el campo BusquedaSimpleAvanzada tiene el valor CARPETA_PERSONAL. 
			// 			Si tiene ese valor es porque viene del nuevo evolutivo de modificar ODEs con versionado.  
			//			Si es así, hay que buscar el idioma del ODE porque lo necesita el servicio de publicación.
			// 			Se invoca al nuevo método crearPifConCuotaYEstado porque permite indicarle el estado en que
			//			se deja la transición.
			OdePublicadoVO odePublicado=null;
			if ((form.getBusquedaSimpleAvanzada()!=null) && (form.getBusquedaSimpleAvanzada().equals("CARPETA_PERSONAL")))
			{
				odePublicado = getSrvPublicacionService().obtenODEPublicado(form.getIdentificadorODE());
				
				form.setIdiomaODE(odePublicado.getIdioma());
				
				resultado = this.getSrvPublicacionService().crearPifConCuotaYEstado(data, LdapUserDetailsUtils.getUsuario(), this.getPropertyValue(MENSAJE_COMENTARIOS_NUEVA_VERSION), form.getIdentificadorODE()+ ".zip", form.getIdiomaODE(),PUBLICADO_VERSIONANDOSE);
