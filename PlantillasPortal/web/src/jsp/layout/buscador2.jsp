...
<c:when test="${!empty form.idiomBuscBackingList}">
   <html:select name="form" property="idiomBusc" styleId="Idioma" title="${i18nValue }" styleClass="caja_buscador_selector">
     <html:optionsCollection name="form" property="idiomBuscBackingList" label="label" value="value"/>
   </html:select>
</c:when>

<c:otherwise>
    <html:select name="form" property="idiomBusc" styleId="Idioma" title="${i18nValue }"/>
</c:otherwise>
