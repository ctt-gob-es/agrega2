...
<!--    <form method="get" action="<rewrite:rewrite url="${initParam.url_buscador}"/>"> -->
	<fieldset id="caja_buscador">
    	<div>
			...
			<c:when test="${!empty form.buscContenido}">
				<input onkeypress="searchKeyPress(event);" onfocus="clearText(this)" type="text" autocomplete="off" id="buscadorGeneral" class="buscador" value="${form.buscContenido}" title="Buscador" name="buscContenido"  />
			</c:when>
			<c:when test="${!empty form.keyword}">
				<input onkeypress="searchKeyPress(event);" onfocus="clearText(this)" type="text" autocomplete="off" id="buscadorGeneral" class="buscador" value="${form.keyword}" title="Buscador" name="buscContenido"  />
			</c:when>
			<c:otherwise>
				<!--
				<input onkeypress="searchKeyPress(event);" onfocus="clearText(this)" type="text" autocomplete="off" id="buscadorGeneral" class="buscador" value="<bean:message key='buscador.textinput'/>" title="Buscador" name="buscContenido"  />
				-->
				<input onkeypress="searchKeyPress(event);" type="text" autocomplete="off" id="buscadorGeneral" class="buscador" value="" title="Buscador" name="buscContenido"  />
			</c:otherwise>
			</c:choose>
			<bean:define id="i18nValue"><bean:message key="buscador.idioma.por.defecto"/></bean:define>
	...
	<logic:notEqual name="form" property="tipoLayoutBuscador" value="BUSCADOR">
		<fieldset class="radios">
			<c:choose>
			<c:when test="${(empty form.tipoBusqueda) or (form.tipoBusqueda!='01' and form.tipoBusqueda!='02' and form.tipoBusqueda!='05' and form.tipoBusqueda!='06')}">
