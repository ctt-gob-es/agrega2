...
public class MultiSearcherSQI {
	 ...
	 * @param nodosSQI nodos sobre la federación donde se va a hacer la llamada
	 ...
	 * @throws Exception
	 */
	//Se agrega el parametro AgregaProperties.IMAGENES_NODOS_SQI_URL para evitar usar el SrvPropiedadService en esta clase
	public static LomEsVO[] searchDocuments(ParamBusquedaSQIVO parametros,List nodosSQI, int maxWaitThread, int maxWaitPool, String imgNodosSqiUrl) throws Exception{ 
...
//			Me recorro todos los documentos resultado de la busqueda.
				...
				for(int j = 0; j < documentos.length ; j++ ){
					...
					if(documentos[j]!=null && 
			    		...
			    		for(int k=0; k < ((LomEsVO[])documentos[j]).length;k++){
			    			((LomEsVO[])documentos[j])[k].setNodo(((NodoSQIVO)nodosSQI.get(indNodosSQI)).getNombreNodo());
			    			//TODO: Le insertaria el path de la imagen aqui, pues tenemos 
			    			if(((NodoSQIVO)nodosSQI.get(indNodosSQI)).getImagenNodo()!=null && !((NodoSQIVO)nodosSQI.get(indNodosSQI)).getImagenNodo().equals("") && ((NodoSQIVO)nodosSQI.get(indNodosSQI)).getImagenNodo().length()>0){
			    				String imagenNodo=imgNodosSqiUrl+"/"+((NodoSQIVO)nodosSQI.get(indNodosSQI)).getImagenNodo();
			    				if (logger.isDebugEnabled()) logger.debug("Le insertamos la direccion de la imagen "+imagenNodo);
			    				((LomEsVO[])documentos[j])[k].setImagen(imagenNodo);
			    			}else{
			    				((LomEsVO[])documentos[j])[k].setImagen(null);
			    			}
		...
		resultado=todos.toArray(new LomEsVO[todos.size()]);
		return resultado;
	}
