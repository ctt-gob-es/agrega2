...
public class MultiSearcherSQIThread extends Thread {
	...
	private static final String LOMES = "LOM-ES";
	private static final String LOM = "LOM";	
	private static final String VSQL = "VSQL";
...
//        	LENGUAJE CONSULTA TODO: vigilar las excepciones que se pueden devolver
		try{
			logger.debug("Vamos a hacer el setQueryLanguage con idSession "+idSession+" y el lenguaje de consulta "+nodo.getLenguajeConsulta()+" llamando al servicio "+servicio);
...
//			EJECUTAMOS LA QUERY TODO: vigilar las excepciones que se pueden devolver
		String sinc="";
		try{
//			servicio.setMaxQueryResults(idSession, new Integer(100));
//			logger.debug("Le insertamos que nos devuelva 100 resultados por nodo");
			sinc = servicio.synchronousQuery(idSession, consulta, 0);
...
//			Hay que extraer la respuesta del formato de respuesta del interfaz SQI
	...
	private String obtenerRespuestaLom(String respuestaSQI)
	{
		...
		StringBuilder stringLoms=new StringBuilder();
		if (respuestaSQI!= null && !respuestaSQI.equals("") && respuestaSQI.length()>0){//Vamos a coger la respuesta y vamos a sacar de ella, solo lo que nos interesa, los loms
			...
			while( i<longitud){
				...
				if(inicio>0){
					...
					stringLoms.append(respuestaSQI.substring(inicio, fin+LOMWrapper.FIN_LOM.length()));
//					if(logger.isDebugEnabled()){
