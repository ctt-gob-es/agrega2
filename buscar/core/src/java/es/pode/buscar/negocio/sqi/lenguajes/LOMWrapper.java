...
public class LOMWrapper {
	...
	protected static Logger logger = Logger.getLogger(LOMWrapper.class);
...
//					logger.debug("El lom ["+lom.substring(inicio, fin+LOMWrapper.FIN_LOM.length())+"]");
			...
			String[] loms=lomsArray.toArray(new String[lomsArray.size()]); 
...
//					-Si no hay descripcion ponemos la lozalizacion como titulo y sin descripcion  
							...
							if(localizacion!=null && !localizacion.equals("")){//La localizacion no puede ser nula, si lo es deshechamos el lom
								...
								if(general!=null){
									...
									if(titulos!=null && titulos.size()>0){
										...
										if(titulo!=null && !titulo.getValor().equals("")){
											...
											if(descripciones!=null && descripciones.size()>0){
												...
												if(descripcions!=null && descripcions.size()>0){													
													LangStringAgrega descripcion=descripcions.get(0);
													if(descripcion!=null){
														...
														if(logger.isDebugEnabled())
															logger.debug("No tenemos descripcion, lo dejamos vacio");
											...
											if(descripciones!=null && descripciones.size()>0){
												...
												if(descripcions!=null && descripcions.size()>0){													
													LangStringAgrega descripcion=descripcions.get(0);
													if(descripcion!=null && !descripcion.getValor().equals("")){//Si no trae titulo, le metemos las primeras 10 palabras de la descripcion(o toda la descripcion si no llega a las 10 palabras)
														String desc=descripcion.getValor();
	 ...
	 * 
	 * */
	protected static String obtenerTituloDeDescripcion(String descripcion) throws Exception{//Le pasamos la descripcion y vamos a recuperarlo para titulo, con la 5 primeras palabras
		...
		try{
			if(descripcionArray.length>PALABRAS_MAX){
				StringBuilder tituloCorto=new StringBuilder();
				for(int i=0;i<PALABRAS_MAX;i++){
					tituloCorto.append(descripcionArray[i]);
				}
				if(logger.isDebugEnabled()){
					logger.debug("La descripcion tiene mas de ["+PALABRAS_MAX+"] palabras, la descripcion es ["+descripcion+"], y el titulo queda ["+tituloCorto+"]");
				}
				return tituloCorto.toString();
			}
			//				tituloCorto=descripcion;
			if(logger.isDebugEnabled()){
				logger.debug("La descripcion tiene menos de ["+PALABRAS_MAX+"] palabras, la descripcion, y en este caso el titulo, es ["+descripcion+"]");
			}
			return descripcion;
		}catch(Exception e){
