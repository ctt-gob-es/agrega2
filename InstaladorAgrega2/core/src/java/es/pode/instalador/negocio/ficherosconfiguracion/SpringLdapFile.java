...
public class SpringLdapFile {
	...
	/**
	 * Array con la lista de las variables que estaban contenidas en el fichero de springldap.xml y que se modificaban en la instalacion.
	 */
	public static String[] variables = new String[]{
		InstalacionVariables.LDAPURL                ,
		InstalacionVariables.LDAPUSUARIOSBINDSUBPATH,
		InstalacionVariables.LDAPPATH               ,
		InstalacionVariables.LDAPMANAGERDN          ,
		InstalacionVariables.LDAPMANAGERPASSWD };
	/**
	...
	public static String directorioDestinoAlternativo 	= "server"+File.separator+"alternativo"+File.separator+"conf";
	...
	public static String ficheroPlantilla = "/springldap.xml";
	
	/**
	 * Lista con las variables que estaban contenidas en el fichero de springldap.xml y que se modificaban en la instalacion.
	 */
	public static List<String> listaVariables = (List<String>) Arrays.asList(variables);
	
	/**
	 * Metodo que indica si la variable que se le pasa es de las contenidas anteriormente en el fichero de springldap.xml
	 * o no.
	 * @param variable Valor de la variable que se quiere averiguar si estaba reflejada en el fichero springldap.xml en la instalacion.
	...
	public static void modificaVariables(PropiedadVO[] prop, String homeFile, String typeJboss)throws Exception
	{
		...
		InputStream ipstrm = SpringLdapFile.class.getResourceAsStream(ficheroPlantilla);
		Sed.sustituir(ipstrm, ficheroDestinoDisco,propiedades);
		return;
	}

	/**
	 * Metodo que chequea que las variables que se le pasan contienen todas las variables necesarias para rellenar con exito el fichero springldap.xml
	 * @param prop
	 */
...
//		Me copio las variables que hacen falta en el fichero
		...
		{	
			...
			throw new Exception("Imposible modificar fichero springldap.xml. El numero de propiedades suministradas no es correcto. Faltan por rellenar ["+copiaVariables.size()+"] valores de variables de ["+variables.length+"] totales. Faltan: "+faltan);
		}
	}
}
