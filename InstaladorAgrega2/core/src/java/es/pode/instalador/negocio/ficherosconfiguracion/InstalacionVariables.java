...
//		IMGSERVERPORT,
    ...
    /**
     * Elimina el prefijo especifico en el nombre de las variables que indica a que JBoss afectan.
     * Como este prefijo es util solo para la DB, es necesario para el resto de operaciones no usarlo.
     * @param props
     * @param tipoJboss
     * 				Su valor debe ser JBOSS_TYPE_DEF, JBOSS_TYPE_ALT o JBOSS_TYPE_NODEX
     * @return
     */
    public static PropiedadVO[] eliminarPrefijoTipoJboss (PropiedadVO[] props, String tipoJboss) {
    	if(props==null || props.length==0) return props;
    	if (esUnTipoJboss(tipoJboss)==false)
    		return props;
	    for(int i = 0; i < props.length; i++) 
	    	props[i].setNombre(props[i].getNombre().replace(tipoJboss, ""));
	    return props;    	
    }
	
	
    /**
     * Metodo que agrega un prefijo especifico al nombre de las variables para saber a que JBoss afectan.
     * Solo añade el prefijo a aquellas variables que son particulares de cada JBoss.
     * Este prefijo solo es util para interactuar con la DB.
     * @param props
     * @param tipoJboss
     * 				Su valor debe ser JBOSS_TYPE_DEF, JBOSS_TYPE_ALT o JBOSS_TYPE_NODEX
     * @return
     */
	public static PropiedadVO[] agregarPrefijoTipoJboss (PropiedadVO[] props, String tipoJboss) {
    	
    	if(props==null || props.length==0) return props;
    	if (esUnTipoJboss(tipoJboss)==false)
    		return props;
    	
    	//Nos esaguramos que la lista de propiedades viene limpia de prefijos que puedan indicar un JBoss
    	for(int i=0; i< tipos_jboss.length; i++)
    		props=eliminarPrefijoTipoJboss(props, tipos_jboss[i]);
	    
    	for(int i=0; i<props.length; i++) {
	    	//Vemos si el nombre de la variable esta en la lista de comunes
    		if(isVariableParticular(props[i].getNombre()))
    			props[i].setNombre(tipoJboss+props[i].getNombre());
	    }
	    return props;    	
    }
