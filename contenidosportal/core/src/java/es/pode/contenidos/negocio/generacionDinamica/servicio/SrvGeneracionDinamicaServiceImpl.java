...
//	   	 	getContenidoODEDao();
    		...
    		*/
			...
			if(contenidos!=null && contenidos.size()>0){
    			...
    			logger.warn("No ha devuelto ningún ODE con la fecha["+fecha+"], vamos a ir mirando los días anteriores" );
...
//    		ContenidoODEVO contenidoODE=new ContenidoODEVO();
	...
	//	   		Modificamos la URL para que coja la imagen grande
				...
				Calendar calendario=Calendar.getInstance();
		   	 	...
		   	 	if(logger.isDebugEnabled()){
		   	 		...
		   	 		logger.debug("Existe ODE para ese día, se va a cambiar por una nueva con hora:["+fecha.toString()+"]");
		   	 	}
	...
	private DocVO30 obtenODEDiario() throws RemoteException, Exception {
		...
		if(odeDiario !=null && i==reintentosInt){
			odeDiario.setImagen(AgregaPropertiesImpl.getInstance().getProperty(AgregaProperties.URL_IMAGEN_DEFECTO_GRANDE));//Le metemos la url de una imagen común para que no nos de error al copiarlo
