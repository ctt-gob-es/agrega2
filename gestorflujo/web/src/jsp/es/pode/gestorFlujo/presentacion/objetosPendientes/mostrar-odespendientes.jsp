...
<tiles:insert definition="layout-administrador">
	...
	<tiles:put name="body" type="string">
		...
		<div>
		<h2><bean:message key="gestorFlujo.mostrarOdes.pendientes.title" /></h2>
		</div>
		<ul>
			<li><a
				href="<html:rewrite action="${initParam.url_despublicados }"/>"><bean:message
		...
		<bean:message key="gestorFlujo.mostrarOdes.pendientes" /></h3>
		<input type="hidden" name="esDespublicadoValueList" value="false" /> 
		...
		</bean:define> <bean:define id="idUsuarioLocal">${form.idUsuario}</bean:define> <bean:define
			id="summary">
			<bean:message key="noticias.tablaAdministracionNoticias" />
		</bean:define> <bean:define id="esDespublicado">${form.esDespublicado}</bean:define>
		<bean:define id="urlCerrar">/${initParam.url_gestorFlujo_init}/${initParam.url_pendientes}</bean:define>
		<display:table name="${form.listaODES}" requestURI="" export="false"
			...
			<display:column sortable="true" sortProperty="titulo" style="valign:top;" class="tar10" title="${tituloValue}" >
				<html:link action="${initParam.url_visualizar_pendientes}?idOde=${fila.idODE}"
			...
			<display:column style="valign:top;" class="tar11">
				<span class="oculto">-</span>
				<html:link
					action="${initParam.url_pendientes_rechazar}?idODE=${fila.idODE}&idUsuario=${idUsuarioLocal}&titulo=${fila.titulo}">
			...
			<display:column style="valign:top;" class="ejec">
				<span class="oculto">-</span>
				<c:choose>
					<c:when test="${form.tipoUsuario > 1}">
						<html:link
							action="${initParam.url_listarDuplicados}?idODE=${fila.idODE}&idUsuario=${idUsuarioLocal}&titulo=${fila.titulo}&esDespublicado=${form.esDespublicado}&retorno=${form.retorno }">
							<bean:message key="gestorFlujo.mostrarOdes.publicar" />
						</html:link>
					</c:when>
					<c:otherwise>
						<i><bean:message key="gestorFlujo.mostrarOdes.publicar" /></i>
					</c:otherwise>
				</c:choose>
