...
<form method="post" id="objetosPersonalesCUMostrarODESPersonalesSubmitForm" action="<html:rewrite action="/ObjetosPersonalesCU/MostrarODESPersonalesBuscarPersonales"/>">
...
<!--  INICIO CAJA DE FORMULARIO   -->
	<h3 class="h3_generico_small"><bean:message key="gestorFlujo.busqueda.personales" /></h3>
...
<!-- !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! -->	  	
			...
			<pg:pager id="pgpersonales" url="" items="${fn:length(form.listaODES)}" maxPageItems="4" maxIndexPages="10" scope="request" export="currentPageNumber=pageNumber">
			<ul id="lista_de_odespersonales">
			<!-- Tengo que pasar una fila de SIN ALBUM -->
			<c:if test="${fn:length(form.listaODES)>0}">
			<c:forEach items="${form.listaODES}" var="personales">
				...
				<div class="globo_blanco_01">
					<div class="globo_blanco_02">
						...
						<div class="li_izq_usuario">
							<img src="${personales.urlImagen }" alt="${personales.titulo}" />
							<br>
							<div style="color: #000000 !important;
								...
								<c:if test="${personales.estaVersionandose=='true'}">	
									<bean:message key="gestorFlujo.mostrarOdes.version" />
								</c:if>
							</div>
						</div>
						<div class="li_der_usuario" >
						
						<a href="<html:rewrite action="/ObjetosPersonalesCU/MostrarODESPersonalesPrevisualizar.do?idOde=${personales.idODE}&comunidadAgrega=false" />" class="tit_obj" target="_blank">${personales.titulo} <c:if test="${personales.estaVersionandose=='true'}"> (<bean:message key="gestorFlujo.mostrarOdes.nueva.version" />)</c:if></a>
