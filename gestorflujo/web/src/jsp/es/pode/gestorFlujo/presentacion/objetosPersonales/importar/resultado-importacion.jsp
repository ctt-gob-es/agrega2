...
<tiles:insert definition="layout-gestor-flujo-con-style">
	...
	<tiles:put name="body" type="string">
		...
		<!-- Aviso de licencias -->	
		<c:if test="${form.mostrarAviso == true}">
			<div class="error">
			<bean:message key="gestorFlujo.importar.aviso.licencias" />
			</div>
		</c:if>
			...
			<c:if test="${result.valido == true}">
				...
				<div id="m${indice}" class="caja_abierta">
				<div class="formu" style="padding-bottom:0;">
				<p class="err_p"><bean:message key="gestorFlujo.importar.error.vocabularios.fechas.continuacion" /></p>
				<ul class="lista_validacion">
		...
		<!--		Boton aceptar		-->
		<form method="post"
			action='<html:rewrite action="/${initParam.url_importar_resultado }"/>'>
		<fieldset class="tipo ft_centrada">
		<input class="boton_125" type="submit"
			value="<bean:message key="gestorFlujo.aceptar"/>" /></fieldset>
			</form>
