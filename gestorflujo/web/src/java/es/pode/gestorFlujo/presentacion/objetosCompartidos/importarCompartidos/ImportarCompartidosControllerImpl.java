...
//						Se lo añado al espacio libre y se lo vuelvo a pasar a la sesion.
					...
					logger.debug("Los atributos que pasamos a la jsp son: Una colleccion:["+resultado+"], con titulo ["+((ResultadoImportacion)resultado.get(0)).getTitulo()+" con mensajes ["+Arrays.toString(((ResultadoImportacion)resultado.get(0)).getMensajes())+" y validacion["+((ResultadoImportacion)resultado.get(0)).getValido());
				} catch (Exception ex) {
