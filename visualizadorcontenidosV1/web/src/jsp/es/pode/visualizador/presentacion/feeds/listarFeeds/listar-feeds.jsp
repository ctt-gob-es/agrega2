...
<c:if test="${param.periodicidad=='no' }">
	...
	<!--  INICIO GLOBO GRIS   -->
	<!--  INICIO GLOBO GRIS   -->
 	<rss:feed  url="${urlObtenerFeed}?idFeed=${param.idFeed}&feedNum=${feedDefault}&idiomaNavegacion=${param.idiomaNavegacion}" feedId="${param.titulo}"/>
 				...
 				<div id="formulario" >
					...
					<div class="caja_tabla bordeada" >
						<table border="1" class="administracion_tareas" cellpadding="0" cellspacing="0" width="100%" summary="Tabla de Lo Más Mostrado">
							...
							<rss:forEachItem feedId="${param.titulo}"> 
								<!--  impar   -->		
 								<c:if test="${contIndicesItem%2!=0}">
									<tr class="tr_gris">
										<td colspan="3" valign="top" align="left"  class="tar2"><span class="bld"><%=contItem %>.</span> &nbsp;<a href="<rss:itemLink feedId="${param.titulo}" />"><rss:itemTitle feedId="${param.titulo}" /></a></td>
									</tr>
								</c:if> 
								<!--  impar   -->		
 								<c:if test="${contIndicesItem%2==0}">
...
<c:if test="${param.periodicidad=='si' }">
	...
	<c:forEach var="repetir" begin="0" end="2">
		...
		<!--  INICIO GLOBO GRIS   -->
		<c:if test="${repetir==0}">
			<rss:feed url="${urlObtenerFeed}?idFeed=${param.idFeed}.1.1&feedNum=${feedDefault}&idiomaNavegacion=${param.idiomaNavegacion}" feedId="${param.titulo}${repetir}"/>
			<c:set value="feeds.agregador.semana" scope="page" var="titulo"/>
		</c:if>
		<c:if test="${repetir==1}">
			<rss:feed url="${urlObtenerFeed}?idFeed=${param.idFeed}.1.2&feedNum=${feedDefault}&idiomaNavegacion=${param.idiomaNavegacion}" feedId="${param.titulo}${repetir}"/>
			<c:set value="feeds.agregador.mes" scope="page" var="titulo"/>
		</c:if>
		<c:if test="${repetir==2}">
			<rss:feed url="${urlObtenerFeed}?idFeed=${param.idFeed}.1.3&feedNum=${feedDefault}&idiomaNavegacion=${param.idiomaNavegacion}" feedId="${param.titulo}${repetir}"/>
					...
					<div id="formulario" >
						...
						<div class="caja_tabla bordeada" >
							<table border="1" class="administracion_tareas" cellpadding="0" cellspacing="0" width="100%" summary="Tabla de Lo Más Mostrado">
								...
								<rss:forEachItem feedId="${param.titulo}${repetir}" startIndex="0" endIndex="10">
									<!--  impar   -->		
									<c:if test="${contIndices%2!=0}">
										<tr class="tr_gris">
											<td colspan="3" valign="top" align="left"  class="tar2"><span class="bld"><%=cont %>.</span> &nbsp;<a href="<rss:itemLink feedId="${param.titulo}${repetir}" />"><rss:itemTitle feedId="${param.titulo}${repetir}" /></a></td>
										</tr>
									</c:if>
									<!--  impar   -->		
									<c:if test="${contIndices%2==0}">
