...
public class GeneradorHtml {
	...
	public static final String SCORM_2004="SCORM2004"; 
	public static final String SCORM_12="SCORM12"; 
	public static final String IMSCP="IMSCP"; 
	...
	public GeneradorHtml() 
	{
		...
		try {
			...
			root.put("solapa.title","Menu");
			
			
			NodeModel.useDefaultXPathSupport();
			cfg.setObjectWrapper(new DefaultObjectWrapper()); 
		} catch (Exception e) {
			logger.error("No se pudo cargar el fichero properties", e);
		}
	...
	throws Exception
	{
		...
		try {
			...
			logger.error("Error en GeneradorHTML:generarFrames .. " , e );
	...
	throws Exception
	{
		...
		}else
		{
			...
			try {
				...
				logger.error("Error en GeneradorHTML:obtenerTitulo .. " , e );
				throw e;
			}
	...
	throws Exception
	{
		...
		try {
			...
			logger.error("Error en GeneradorHTML:generarCabecera .. " , e );
			throw  e;
		}finally{
			index=null;
			if(fo!=null)
	...
	throws Exception
	{
		...
		try {
			...
			logger.error("Error en GeneradorHTML:generarBlanco .. " , e );
			throw  e;
		}finally{
			index=null;
			if(fo!=null)
	...
	throws Exception
	{
		...
		try {
			...
			logger.error("Error en GeneradorHTML:generarContenidoFrame .. " , e );
			throw  e;
		}finally{
			index=null;
			if(fo!=null)
	...
	throws Exception
	{
		...
		try {
			...
			logger.error("Error en GeneradorHTML:generarContenidoIndex .. " , e );
			throw  e;
		}finally{
			index=null;
			if(fo!=null)
	...
	throws Exception
	{
		...
		try {
			...
			if(GeneradorHtml.SCORM_12.equals(this.formato))
				temp = cfg.getTemplate("menuXML-SCORM12.ftl");
			else if(GeneradorHtml.IMSCP.equals(this.formato))
				temp = cfg.getTemplate("menuXML-IMSCP.ftl");
			else
			...
			logger.error("Error en GeneradorHTML:generarMenu .. " , e );
			throw  e;
		}finally{
			index=null;
			if(fo!=null)
	...
	throws Exception
	{
		...
		crearCarpeta(dirDestino+ "/catalogacion");
		String pathManifest=pathOde+MANIFEST;
		try {
			modelo= NodeModel.parse(new File(pathManifest));
		} catch (Exception e) {
			logger.error("ERROR al parsear el manifest "+pathManifest);
			throw e;
		}
		root.put("doc",modelo);
	...
	throws Exception
	{
		...
		try {
			if(pathLomesExterno!=null)
			{
				lomes= new File(dirDestino + "/catalogacion/catalogacionLomes.xml");
				if(!lomes.exists())
					lomes.createNewFile();
			}
			...
			logger.error("Error en GeneradorHTML:obtenerLOMES .. " , e );
			throw e;
		}finally{
			lomes=null;
			if(fo!=null)
	...
	throws Exception
	{
		...
		try {
			...
			logger.error("Error en GeneradorHTML:chequearTipoLomesExterno .. problemas al leer fichero lomes externo " , e );
			throw e;
		}
		return resultado;
	}
	...
	throws Exception
	{
		...
		try {
			...
			logger.error("Error en GeneradorHTML:generarIndex " , e );
			throw  e;
		}finally{
			index=null;
			if(fo!=null)
	...
	public void generarIndexCompleto(
			String pathOde, 
			String dirDestino)
	throws Exception
	{
		...
		try {
			...
			logger.error("Error en GeneradorHTML:generarIndexCompleto .. " ,e );
			throw new Exception(e);
		}
	}
	
	
	private void generarHtmlCompleto()
	throws Exception
	{
		FileOutputStream fo=null;
		Writer out=null;
		...
		try {
			...
			if(SCORM_12.equals(this.formato))
				temp = cfg.getTemplate("index_completo_SCORM12.ftl");
			else if(IMSCP.equals(this.formato))
				temp = cfg.getTemplate("index_completo_IMSCP.ftl");
			else
				temp = cfg.getTemplate("index_completo_SCORM2004.ftl");
			...
			logger.error("Error en GeneradorHTML:generarHtmlCompleto .. " , e );
			throw  e;
		}finally{
			index=null;
			if(fo!=null)
