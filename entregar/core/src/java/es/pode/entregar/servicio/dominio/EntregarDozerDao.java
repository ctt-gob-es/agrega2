...
public class EntregarDozerDao {
	...
	private static Logger logger = Logger.getLogger(EntregarDozerDao.class);
		...
		for (int i = 0; i < iLength; i++) {
			...
			for (int cont =0; cont<arrayItem.length;cont++){
				...
				try {
					...
					logger.debug("Recurso " + item.getIdItem_ref() + " no encontrado");
				}
			...
			try {
				...
				logger.debug("Error obteniendo Secuencia:",e);
			}
		...
		if (manifest!= null) {
			...
			for (int r=0; r<res.getResourceCount(); r++) {
				...
				if ((base!=null) && (!base.equals(""))) {
					...
					if (!((href.toLowerCase().indexOf(PROTOCOLO_HTTP) >= 0) || (href.toLowerCase().indexOf(PROTOCOLO_HTTPS) >= 0 ))){
		...
		if (orgs!=null) {
			...
			try {
				...
				if (secuenciaOrg !=null) {
					...
					controlMode.setChoice(cMode.getChoice());
					controlMode.setChoiceExit(cMode.getChoiceExit());
					controlMode.setFlow(cMode.getFlow());
					controlMode.setForwardOnly(cMode.getForwardOnly());
				...
				throw new Exception("Error obteniendo Secuencia ",e);
			}
			 // -- rellenamos opciones de navegacion --
			orgs.setClicable(Boolean.TRUE);
	   ...
	   throws Exception, ParseadorException{
		...
		if (arrayItems !=null){
		  for (int it=0; it<arrayItems.length; it++ ){
			  ...
			  arrayItems[it].setClicable(Boolean.TRUE);
