...
public class ConfigurarModificacionControllerImpl extends ConfigurarModificacionController
{
    ...
    public final void eliminarCambios(ActionMapping mapping, es.pode.modificador.presentacion.configurar.modificacion.EliminarCambiosForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
    {
    	...
    	if(posiciones!=null) {
    		...
    		for(int i=0;i<posiciones.length;i++) {
    			paraBorrar.add(cambios.get(posiciones[i]));
    		}
	...
	public void guardarModificacion(ActionMapping mapping, GuardarModificacionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		if((configuracion.getCambios()!=null && configuracion.getCambios().getCambios()!=null && configuracion.getCambios().getCambios().length>0)&&
				((configuracion.getObjetos()!=null && configuracion.getObjetos().getObjetos()!=null && configuracion.getObjetos().getObjetos().length>0)||(configuracion.getObjetos()!=null && configuracion.getObjetos().getPaths()!=null && configuracion.getObjetos().getPaths().length>0))){
			...
			logger.error("Tiene que añadir objetos y añadir cambios para poder guardarlo");
			throw new ValidatorException("{configurarModificacion.cambios.validar.guardar.error}");
		}
	}
	...
	public void modificarCambio(ActionMapping mapping, ModificarCambioForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(sPosicion!=null) {
			...
			if(logger.isDebugEnabled())logger.debug("Recuperando cambio " + posicicion + " del array de cambios");
			...
			cambioSession.setPosicion(posicicion);
