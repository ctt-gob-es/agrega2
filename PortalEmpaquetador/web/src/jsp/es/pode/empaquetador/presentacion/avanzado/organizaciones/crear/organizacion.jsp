...
<bean:define id="crearOrgSesion" name="<%=es.pode.empaquetador.presentacion.avanzado.organizaciones.crear.CrearOrganizacionSession.SESSION_KEY%>" scope="session"/>

<c:choose>
<c:when test="${!empty crearOrgSesion.title}">

<bean:define id="urlAsistente"><html:rewrite action="/GestorOrganizaciones/GestorOrganizacionesModificar"/>?identifier=${crearOrgSesion.identifier}</bean:define>
</c:when>
<c:otherwise>

<bean:define id="urlAsistente"><html:rewrite action="/GestorOrganizaciones/GestorOrganizacionesSubmit"/>?action=Crear Organizacion</bean:define>
</c:otherwise>
</c:choose>
