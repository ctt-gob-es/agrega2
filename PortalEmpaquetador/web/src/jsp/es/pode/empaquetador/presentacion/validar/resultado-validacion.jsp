<%@ taglib uri="http://struts.apache.org/tags-tiles" prefix="tiles" %>
<%@ taglib uri="http://struts.apache.org/tags-bean" prefix="bean" %>
<%@ taglib uri="http://struts.apache.org/tags-html" prefix="html" %>
<%@ taglib uri="http://struts.apache.org/tags-logic" prefix="logic" %>
<%@ taglib uri="/WEB-INF/tlds/tags-empaquetador.tld" prefix="emp" %>
<bean:define id="urlArchivos"><html:rewrite action="/GestorArchivos/GestorArchivos"/></bean:define>
<bean:define id="urlRecursos"><html:rewrite action="/GestorRecursos/GestorRecursos"/></bean:define>
<bean:define id="urlOrganizaciones"><html:rewrite action="/GestorOrganizaciones/GestorOrganizaciones"/></bean:define>
<bean:define id="urlSubmanifiestos"><html:rewrite action="/GestorSubmanifiestos/GestorSubmanifiestos"/></bean:define>
<bean:define id="urlGuardar"><html:rewrite action="/GestorArchivos/GestorArchivos"/></bean:define>
<bean:define id="urlAsistente"><html:rewrite action="/GestorArchivos/GestorArchivos"/></bean:define>
<bean:define id="urlEstructura"><html:rewrite action="/GestorBasico/GestorBasico"/></bean:define>
<bean:define id="selected" value="none"/>
<emp:layout selected="${selected}" urlEstructura="${urlEstructura}" urlArchivos="${urlArchivos}" urlRecursos="${urlRecursos}" urlSubmanifiestos="${urlSubmanifiestos}" urlOrganizaciones="${urlOrganizaciones}" urlGuardar="${urlGuardar}" urlAsistente="${urlAsistente}" titleKey="title.Comun">


<!--  INICIO CAPA MADRE   -->

<!-- XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX -->
<!-- XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX -->
<!-- XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX -->
<!-- XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX -->
<!-- ATENCION!! RREMPLAZAR ESTA CAPA DE LA  PLANTILLA DE CONTENIDO ("plantilla_contenido") POR LA QUE SE DESEE EN LA PLANTILLA BASE  -->
<!-- Inicio plantilla contenido  -->
<!-- Inicio plantilla contenido  -->
<div class="plantilla_contenido">
<%@ include file="/layout/messages.jsp" %>
	<h2><bean:message key="validacion.titulo"/></h2>
<form id="validarValidacionErrorCerrarForm" action='<html:rewrite action="/Validar/ValidacionErrorCerrar"/>' method="post">


<!--  INICIO GLOBO GRIS   -->
<!--  INICIO GLOBO GRIS   -->
<div class="globo_gris" >
	<div class="globo_gris_01">
		<div class="globo_gris_02">
			<div class="globo_gris_03">
				<div id="formulario" class="ali_c">					
				<br />
					
				<logic:equal name="form" property="hayAlgunError" value="true">
						<p><em class="incorrecto"><bean:message key="validacion.noCorrectamente"/></em></p>
				</logic:equal>
				<logic:equal name="form" property="hayAlgunError" value="false">	
						<p><em class="correcto"><bean:message key="validacion.correctamente"/></em></p>
				</logic:equal>
				
				<br />
				</div>				
			</div>
		</div>
	</div>
</div>
<!--  FIN GLOBO GRIS   -->
<!--  FIN GLOBO GRIS   -->
					
				
<logic:equal name="form" property="hayAlgunError" value="true">		
	<!--  FIN GLOBO GRIS   -->
	<!--  FIN GLOBO GRIS   -->
	<p class="parrafo_comun" id="separacion3"><bean:message key="validacion.detalles.errores"/></p>
	<!--  INICIO GLOBO GRIS   -->
	<!--  INICIO GLOBO GRIS   -->
	<div class="globo_gris" >
		<div class="globo_gris_01">
			<div class="globo_gris_02">
				<div class="globo_gris_03">
					<div id="formulario_02" >
					<br />
				
					<logic:equal name="form" property="hayErrorCatalogacion" value="true">
						<!--  INICIO CAJA DE FORMULARIO DE ERRORES CATALOGACION -->
						<ul class="lista_validacion">			 
							<logic:iterate id="mensaje"  name="form" property="mensajesErrorCatalogacion">
							<li>${mensaje}</li>	 
							</logic:iterate>
						</ul>
						<!--  FIN CAJA DE FORMULARIO DE ERRORES CATALOGACION -->
					</logic:equal>
					
					<logic:equal name="form" property="hayErrorLicencias" value="true">
						<!--  FIN CAJA DE FORMULARIO DE ERRORES LICENCIAS -->	
						<ul class="lista_validacion">			 
							<li><bean:message key="validacion.licencias.error"/>: ${form.mensajesErrorLicencias}</li>
						</ul>
						<!--  FIN CAJA DE FORMULARIO DE ERRORES LICENCIAS -->
					</logic:equal>
					
					<br />
					</div>
				</div>
			</div>
		</div>
	</div>
	<!--  FIN GLOBO GRIS   -->
	<!--  FIN GLOBO GRIS   -->		
</logic:equal>


<!-- Inicio Botones  -->
<!-- Inicio Botones  -->
<bean:define id="cerrarValue"><bean:message key="validacion.volver"/></bean:define>	
<html:submit styleClass="boton_125 tipo ft_centrada"  value="${cerrarValue}" ></html:submit>
<!-- Fin Botones  -->
<!-- Fin Botones  -->
</form>
</div>
<!-- Fin plantilla contenido  -->
<!-- Fin plantilla contenido  -->
<!-- XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX -->
<!-- XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX -->
<!-- XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX -->

<!--  FIN CAPA MADRE   -->
</emp:layout>