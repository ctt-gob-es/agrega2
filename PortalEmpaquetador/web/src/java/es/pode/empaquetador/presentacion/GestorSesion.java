...
public class GestorSesion {
	...
	public void comprobarPendientesCatalogacion(HttpSession session) {
		...
		if (empSession != null
		 ...
		 //Si ha hecho esto, el ode ha sido catalogado ahora mismo, activams el cambio de modificado en session
		 empSession.setModificado(true);
    ...
    throws java.lang.Exception
{
	...
	public GrupoVO[] buscarHijosIdCollection(HttpServletRequest request) throws Exception {
		...
		Object ultimoIdCollection=null;
		if (getEmpaquetadorSession(request).getIdCollection().size()>0) {
			ultimoIdCollection = this
					.getEmpaquetadorSession(request)
					.getIdCollection()
					.get(this.getEmpaquetadorSession(request).getIdCollection()
							.size() - 1);
		}
		GrupoVO[] hijos = null;
