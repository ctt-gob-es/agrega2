...
//		 Reinicializa el portapapeles a lista vacia
	    ...
	    sesion.setModificado(true);//se ha modificado
	}
	...
	public final void recuperarDatos(
		...
		if (listId != null && !listId.isEmpty()) {
			Object idActual = listId.get(listId.size() - 1);
			...
			if (idActual instanceof OrganizacionVO) {
				...
				grupos.addAll(java.util.Arrays.asList(orgPrincipal.getGrupos()));
				seq = gestorSesion.obtenerCM(orgPrincipal.getSecuencia());
			}
		}
		form.setPortapapeles(this.getEmpaquetadorSession(request).getPortapapeles());
		form.setIdCollection(this.getEmpaquetadorSession(request).getIdCollection());
...
//				throw new ValidatorException("{comunes.licenciaNoPermiteObraDerivada}");
	...
	public final void eliminar(ActionMapping mapping,
		...
		for(int i=0;i<idEliminar.size();i++) {
			...
			this.getEmpaquetadorSession(request).setModificado(true);//indicams que ha sido modificado
		}
	}
...
//		String resultadoAsistente=asBasico.asistenteBasico(empSes, sesArch);
