...
<%@page import = "java.util.*,java.io.*,es.pode.adl.sequencer.*,es.pode.adl.util.*,
es.pode.adl.validator.contentpackage.*,es.pode.adl.server.*,es.pode.adl.client.*,org.apache.log4j.Logger"%>
...
//                     +"AND ItemIdentifier = ?";
	...
	Logger mLogger = Logger.getLogger("sequencingengine.jsp");
   ...
   { 
       ...
       else
       {
            ...
            String rutaOdes = session.getAttribute("webPath") + File.separator + session.getAttribute("idSession")
            + File.separator + session.getAttribute("COURSEID");
          
            System.out.println("rutaOdes " + rutaOdes);

        	 FileInputStream in = new FileInputStream(rutaOdes + "/serialize.obj");
                 ...
                 // session (or auto) or resume
            {
               ...
               {
            	  //AQUI ENTRA CUANDO DAS AL BOTON DE SUSPEND Y LUEGO VUELVES A ENTRAR, ENTRA POR AQUI Y 
            	  //RECUPERA LA ACTIVIDAD EN LA QUE ESTABAS...
               ...
               //                              + "CourseID = ?";                  
                ...
                //nosotros aqui tenemos que modificar el de la session
                /////////////////////////////// repetimos el codigo ver luego si podemos meter en metodo
              
				vLaunchData=(Vector)request.getSession().getAttribute("launchdataList");
                int v=0;
                String itemid;
                boolean encontrado=false;
           		if (mlaunch.mActivityID!=null) {
	                while ((v< vLaunchData.size()) && (!encontrado))
	           		{
		              LaunchData ld = (LaunchData)vLaunchData.get(v);
		            	itemid=ld.getItemIdentifier();
		            	if (itemid.compareTo(mlaunch.mActivityID)==0) {
		            		ld.setSuspendAll(true); //modificamos el suspendAll como lo hacia antes la bbdd
		            	}
		            	v++; //incrementamos contador
	          	 	}//fin modificacion suspendAll!!
           		}
               }//end suspendAll
...
//            }
            int vl=0;
            if (mlaunch.mActivityID!=null){
	            while ((vl< vLaunchData.size()) && (!matched))
	            {
	            	LaunchData ld = (LaunchData)vLaunchData.get(vl);
	            	itemID=ld.getItemIdentifier();
	            	if (itemID.compareTo(mlaunch.mActivityID)==0) {
	            		matched=true;
	            		nextItemToLaunch= ld.getLocation(); //pero no lleva la ruta entera /adl/....
	            		if (ld.getContinue()) {
	            			isNextAvailable = false;
	            		}
	            		if (ld.getPrevious()) {
...
//      LMSDBHandler.closeConnection();
      ...
      if ( specialState.equals( LAUNCH_COURSECOMPLETE )  )
      {
          ...
          response.sendRedirect("endsession.htm"); 
      }
...
%> 
   ...
   <body bgcolor="#FFFFFF" onload="initLMSFrame();">    
      <script language="javascript">            
         ...
         if ( suspendCtrl )
         {
            //window.top.frames['LMSFrame'].document.forms['buttonform'].suspend.style.visibility = "visible";
...
<%
   ...
   { 
	  mLogger.error("Sequencingengine.jsp Exception Occurred: " + e + " !!");
      e.printStackTrace();
   } 
%>
