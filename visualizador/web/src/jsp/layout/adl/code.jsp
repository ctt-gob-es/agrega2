...
<%@page import = "java.util.*,es.pode.adl.sequencer.*,java.io.*, 
               java.lang.Boolean.*, es.pode.adl.util.*,org.apache.log4j.Logger" %>
...
<%
	Logger mLogger = Logger.getLogger("code.jsp");
   ...
   mLogger.info("estamos en code.jsp courseid " + courseID);
   ...
   //      File.separator + courseID + File.separator + "serialize.obj");
      ...
      FileInputStream in = 
    	  new FileInputStream(rutaOdes + "/serialize.obj");

      ObjectInputStream ois = new ObjectInputStream(in);
      activityTree = (SeqActivityTree)ois.readObject();
//    Clear the tree session state
      activityTree.clearSessionState();
      ois.close(); 
      in.close();
    
      sequencer.setActivityTree(activityTree);
     mLogger.info("RutaOdes es  " + rutaOdes);
      mLogger.info("ASC --  ValidRequests " + ValidRequests.mChoice + " " + ValidRequests.mContinue);
      sequencer.getValidRequests(ValidRequests);
      mLogger.info("ASC ---- ValidRequests.mTOC " + ValidRequests.mTOC);
   ...
   {  // get the menu items from the list which came from the sequencer
      ...
      if (( hasAMenu ) && (TOCList !=null))
      {
    	  mLogger.info(" code.jsp  TOCList.size " + TOCList.size());
            ...
            {    toc_item =  (ADLTOC)TOCList.elementAt(j);
               ...
               {  
                  title_vector.addElement(toc_item.mTitle);
                 	mLogger.info("Valor de title_vector en code.jsp " + j + " valor " + toc_item.mTitle);
   ...
   {
	   mLogger.error("problem in TOC  " + e); 
      e.printStackTrace();   
   }
%>  
...
<% 
	mLogger.info("title_Vector size  "  + title_vector.size());
...
<body onload="MTMDisplayMenu()" bgcolor="#FFFFFF" text="#black" link="yellow" 
