...
public class Interceptor 
{
	...
	public static DatosVO getDatosInterceptados(MethodInvocation invocation)
	{
		...
		try {			
	    	...
	    	if(logger.isDebugEnabled())logger.debug("Servicio web en ejecución: " + servicioWebClass);
	        ...
	        if(logger.isDebugEnabled())logger.debug("Metodo del servicio web en ejecución: <" + metodoServicioWeb+">");       
	        ...
	        if (argumentos != null) {	        	
				...
				for(int i = 0; i < argumentos.length; i++) {        
					if (argumentos[i]!=null) {
						...
						if(logger.isDebugEnabled())logger.debug("Argumento (Clase) del metodo del servicio web en ejecución: <" + argumentos[i].getClass().getName() + "> posiciOn del argumento: <" + i+">");
						
						Element argumentoServicioWeb = esInterceptadoArgumento(argumentos[i].getClass().getName(), 
								elementMetodoServicioWeb, i);
    ...
    {        	
        ...
        if(logger.isDebugEnabled())logger.debug("El servicio web " + servicioWebClass + " método " + metodoServicioWeb + " no retorna nada");
...
//			if(logger.isDebugEnabled())logger.debug("Atributo: " + atributos[j]);
	...
	public static Element esInterceptadoMetodo(String metodoServicioWeb, Element elServicioWeb) {
		...
		while (itMetodos.hasNext()) {
			...
			if(logger.isDebugEnabled())logger.debug("Método del servicio web en el XML a comparar: " + el.getAttribute(RegistroCtes.ATTRIBUTE_NAME).getValue());
			
			if (el.getAttribute(RegistroCtes.ATTRIBUTE_NAME).getValue().equals(metodoServicioWeb)) {	
				if(logger.isDebugEnabled())logger.debug("Método del servicio web que se intercepta: " + metodoServicioWeb);
	...
	/**
	 * Comprobación de si hay que interceptar el argumento del método de
	 * servicio web a partir del fichero xml de configuración
	 * 
	 * @param argumentoMetodoServicioWeb
	 *            Nombre del argumento a buscar
	 * @param elMetodoServicioWeb
	 *            El elemento del método del servicio donde se busca el
	 *            argumento
	 * @param pos
...
//					if(logger.isDebugEnabled())logger.debug("Argumento del método del servicio web (XML) a comparar: " + argumentoElement.getAttribute(RegistroCtes.ATTRIBUTE_NAME).getValue());
				if (checkAttribute(argumentoElement, argumentoMetodoServicioWeb, pos)) {
//					if (log.isDebugEnabled())
//						if(logger.isDebugEnabled())logger.debug("Argumento del método del servicio web que se intercepta: " + argumentoMetodoServicioWeb);
	...
	private static boolean checkAttribute(Element attributeElement, String argumentoMetodoServicioWeb, int pos) {
		 ...
		 * solo comparo el nombre (TODO Esto debería revisarse, siembre deberia
