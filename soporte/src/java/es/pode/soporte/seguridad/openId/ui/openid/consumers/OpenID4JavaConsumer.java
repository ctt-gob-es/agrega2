...
public class OpenID4JavaConsumer implements OpenIDConsumer {
    ...
    private static Logger log = Logger.getLogger(OpenID4JavaConsumer.class);
    ...
    public String beginConsumption(HttpServletRequest req, String identityUrl, String returnToUrl) throws OpenIDConsumerException {
       /// return beginConsumption(req, identityUrl,  returnToUrl, returnToUrl);
    	...
    	try
    	{
    		realm = "http://" + this.getAgregaPropertyValue(AgregaProperties.HOST) + this.getAgregaPropertyValue(AgregaProperties.PUERTO) + cadenaRealm;
	...
	{
//   	if (log.isDebugEnabled())log.debug("getAgregaPropertyValue");
		// devolvemos la propiedad
//		if (log.isDebugEnabled())log.debug("properties.getProperty(sKey) "+properties.getProperty(sKey));
