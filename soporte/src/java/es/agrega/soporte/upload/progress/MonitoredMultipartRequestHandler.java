...
/**
 * Clase que extiende la funcionalidad de MultipartRequestHandler de Struts 1.x
 * para la inclusion de un ProgressListener de commons-fileupload en el proceso
 * de los formularios multipart.
 * 
 * @see org.apache.struts.upload.MultipartRequestHandler
 */
public class MonitoredMultipartRequestHandler implements MultipartRequestHandler {
     ...
     */
    private static Logger log = Logger.getLogger(MonitoredMultipartRequestHandler.class);
    ...
    public void handleRequest(HttpServletRequest request)
		...
		/*
		 * Chequeo el parametro progressMonitor en la request. Si existe, lo
		 * meto en sesion para la barra de progreso.
		 */
        String name = request.getParameter("progressMonitor");
        if(log.isDebugEnabled()) log.debug("progressMonitor="+name);
        if(name!=null) {
	      //Create a progress listener
	        org.apache.commons.fileupload.ProgressListener progressListener = new UploadProgressMonitor();
	        upload.setProgressListener(progressListener);
	        request.getSession().setAttribute(name, progressListener);
        }
        ...
        try {
            ...
            if(log.isDebugEnabled()) log.debug("Borrando monitor de progreso de la sesion (" + name + ")");
        	if(name!=null) request.getSession().removeAttribute(name);
