...
//		Set ids = map.keySet();
		...
		for (Iterator<Entry<String, Lom>> iterator = entrySet.iterator(); iterator.hasNext();) {
			Map.Entry<String, Lom> entry = iterator.next();
...
//		}
		result.setId(idArray.toArray(new String[idArray.size()]));
		result.setLomArray(lomes.toArray(new Lom[lomes.size()]));
		return result;
	}
	
	/**
	 * @param lomes Lom[]
	 * @return CambioLomesResult
	 * @throws Exception 
