  ...
  // --------- Default Constructor ----------
	        ...
	        if (msg instanceof ObjectMessage)
	        {
	        	...
	        	if (msgS instanceof Long)
		        {
	        		if(!DecisorOffline.esOffline()) {
		        		logger.debug("idModificacion recuperada del mensaje = " + msgS);
    ...
    public java.lang.Boolean ejecutarTareaModificacion(java.lang.Long idModificacion)
    {
    	...
    	if(modificaciones!=null) {
    		logger.info("Se ejecuta la tarea de modificacion "+idModificacion);
	    	...
	    	try
	    	{
		    	...
		    	if(cambios.getOdes()!=null && cambios.getOdes().length>0) {
						...
						if (publicados.size() > 0) {
							...
							try {
								...
								if (publicador == null) {
									...
									//Llamamos a entregar para eliminar los zip de la descargas
									logger.debug("Se eliminan los zip generados en la descarga de los odes modificados");
									SrvEntregarService servicioEntregar=this.getSrvEntregarService();
									servicioEntregar.eliminarTemporales(idsArray);
									logger.debug("Hemos eliminado los ficheros temporales de los odes del repositorio modificados");
								}
							} catch (Exception e) {
								logger
			    	...
			    	if(finalizadas>0){
			    		if(cambios.isDestructiva())
			    		{
			    			logger.debug("es destructiva el estado final sera pendiente");
			    			modificaciones.setResultado(EstadosTarea.PENDIENTE);
			    			modificaciones.setMsgError(CodigosMensajesError.TAREA_EJECUTADA_PENDIENTE);
			    		} else if(cambios.isSoloValidacion()) {
			    			logger.debug("solo valida, comprobamos si hay zips que no son ODEs en "+path);
					    	Folder[] carpetas=task.getObjetos().getPaths();
					    	int ficherosZip=0;
					    	for (int i = 0; i < carpetas.length; i++) {
								Folder carpeta = carpetas[i];
								ficherosZip=+UtilesFicheros.obtenerZipsDePath(carpeta.getPath(),true).length;
							}
			    			if(ficherosZip!=resultadoCambios.length) {
			    				logger.debug("Hay "+ficherosZip+" ficheros Zip y tengo "+resultadoCambios.length+" resultados");
			    				modificaciones.setResultado(EstadosTarea.WARNING);
			    				modificaciones.setMsgError(CodigosMensajesError.ZIPS_NO_ODE);
			    			}
			    		}
...
java.io.InputStream srvEntregarServiceInputStream = SrvTareasModificacionServiceImpl.class
		.getClassLoader()
		.getResourceAsStream(srvEntregarServiceFile);
logger.debug("srvEntregarServiceInputStream = " + srvEntregarServiceInputStream );
...
srvEntregarServiceEndPointAddress = (String) srvEntregarServiceProperties
		.get("srvEntregarServicePort");
...
logger.debug("srvEntregarServiceEndPointAddress = " + srvEntregarServiceEndPointAddress );
if (srvEntregarServiceEndPointAddress.length() > 0)
	((es.pode.entregar.negocio.servicios.SrvEntregarServiceServiceLocator) srvEntregarService)
			.setSrvEntregarServiceEndpointAddress(srvEntregarServiceEndPointAddress);

es.pode.entregar.negocio.servicios.SrvEntregarService port = srvEntregarService
		.getSrvEntregarService();
logger.debug("retorno : srvEntregarService = " + srvEntregarService );
return port;
