...
//			this.getSrvGestorManifestService().agregarManifiestoZIP(idODE, paquete, idDestino);
			ByteArrayOutputStream byteStream= (ByteArrayOutputStream) paquete.getOutputStream();
			this.getSrvGestorManifestService().agregarManifiestoZIPByte(idODE, byteStream.toByteArray(), idDestino);
		} else {
...
//		String tipoEmpaquetador = LdapUserDetailsUtils.getEmpaquetador().toLowerCase();
	...
	protected String handlePrepararMetadatos(String identificador) throws Exception 
	{
		...
		// Obtengo el identificador del manifest
		Lom lom = manifestAgrega.obtenerLom(manifestImportado.getIdentifier(), null);
...
//				new Locale(idioma), identificador);
	...
	private void generarManifestComun(String identificador, String[] files,
		...
		logger.debug("Generamos identificador para el item");
		String newItemId = PodeUUIDGenerator.getItemUUID(String
				.valueOf(java.lang.Math.random()));
		logger
				.debug("Añadimos item a la organizacion con el recurso anterior");
		...
		item.setTitle("Item");
		org.addItem(item);
		logger.debug("Item añadido a la organizacion");
		/*
...
//	}
	...
	protected void handleGenerarManifestLom(String identificador,
		...
		logger.debug("El idioma que llega es "+idioma);
		...
		logger.debug("El idiomaDestinatario que llega es "+idiomaDestinatario);
		...
		logger.debug("El tipo que llega es "+tipo);
		...
		try {
			...
			String idiomaCatalog=LdapUserDetailsUtils.getIdioma()!= null && LdapUserDetailsUtils.getIdioma().equals(VACIA)?LdapUserDetailsUtils.getIdioma():"es";
			logger.debug("Generamos metadatos con idiomaCatalog "+idiomaCatalog);
	...
	@Override
	protected OdeVO handleEditarODE(String identificador, String tipoEmpaquetador) throws Exception {
		OdeVO ode=null;
    	if(TipoEmpaquetador.AVANZADO.toString().equalsIgnoreCase(tipoEmpaquetador)) {
    		ode=getSrvGestorManifestService().editarODE(identificador);
    	} else {
    		ode=getSrvEmpaquetadorBasicoService().editarOde(identificador);
    	}
    	return ode;
	}
