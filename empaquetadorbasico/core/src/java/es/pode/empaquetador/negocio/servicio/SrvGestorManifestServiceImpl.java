...
public class SrvGestorManifestServiceImpl extends
	...
	private static final String ODE_SIN_TITULO = "ode.sin.titulo";
...
//		logger.debug("voy a transformar el grupoVO pasado por parámetro en Item");
	...
	protected String handleModificarGrupo(
		...
		logger.debug("Modificamos grupo con identificador= "
				+ grupoVO.getIdentifier()+"y el elemento referenciado es "+grupoVO.getElementoReferenciado());
		Item nuevoItem =manifestAgrega.obtenerItem(grupoVO.getIdentifier());
		this.getBeanMapper().map(grupoVO,nuevoItem);
		if(grupoVO.getElementoReferenciado()==null||grupoVO.getElementoReferenciado().equals("")){
			logger.debug("El grupo no tiene asociado ningun item "+grupoVO.getIdentifier());
			nuevoItem.setIdentifierref(null);
		}
	 ...
	 * @param identificador
	 *            Identificador del ODE en edición.
	 * @param cambioManifest
	 *            si se le da valor true se creara un backup del manifest antes de guardar.
	 * @see es.pode.empaquetador.negocio.servicio.SrvGestorManifestService#guardarManifiesto(java.lang.String)
	 */
	@Override
	protected void handleGuardarManifiesto(String identificador, boolean cambioManifest)
			throws java.lang.Exception {
		
		handleGuardarManifiestoControlandoBackups(identificador, cambioManifest, !cambioManifest);
	}
	 ...
	 * @param identificador
	 *            Identificador del ODE en edición.
	 * @param hacerBackupManifest
	 *            si se le da valor true se creara un backup del manifest antes de guardar.
	 * @param hacerBackupODE
	 *            si se le pasa valor true se realizara un backup de los contenidos previos del ODE.
	 * @see es.pode.empaquetador.negocio.servicio.SrvGestorManifestService#guardarManifiesto(java.lang.String)
	 */
	@Override
	protected void handleGuardarManifiestoControlandoBackups(
			String identificador, Boolean hacerBackupManifest,
			Boolean hacerBackupODE) throws Exception {
		...
		File fichero_backup= null;	
		
		
		// Si el parametro booleano es true, hacemos una copia del manifest q habia
		if (hacerBackupManifest){
			String ruta_backup = localizador.getPath() + BARRA + SrvGestorManifestServiceImpl.manifest_backup;
			fichero_backup= new File(ruta_backup);
			if (fichero.isFile()){
				UtilesFicheros.copiar(fichero, fichero_backup);
			}
		}
		...
		try {
			...
			logger.error("no se ha podido guardar el ode: ", e);
			throw new AlmacenamientoException("no se ha podido guardar el ode: " + identificador, e);
		}
...
//			ManifestAgrega ma = new ManifestAgrega(manifest);
			...
			if (titulo == null) {
				titulo = obtenerTituloProvisional(ODE_SIN_TITULO, new Locale(idioma), identificador);
			}

//			String comentarios = VACIA;
			// Si no se ha catalogado, el titulo es null y no se modificara.
			if (logger.isDebugEnabled())
				logger.debug("Guardando cambios en publicacion para el manifest "
								+ identificador
								+ " con usuario "
								+ user
								+ " e idioma "
			...
			String ComentarioModificacion=VACIA;
			//Tengo que ver si el ode era un despublicado por contenido inapropiado, que se le deje su comentario
			es.pode.publicacion.negocio.servicios.TransicionVO[] transiciones=this.getSrvPublicacionService().obtenHistorialPorIdODE(identificador);
			if (transiciones!=null && transiciones.length>0){
			    String comentarios= transiciones[0].getComentarios();
				String nombreFichIdioma="/empaquetador_"+idioma+".properties";
				InputStream in = this.getClass().getResourceAsStream(nombreFichIdioma);
				Properties propIdioma = new Properties();
				propIdioma.load(in);
				String comentarioProp=propIdioma.getProperty("COM_OK_CI");					
				propIdioma.load(in);
				in.close();
				
				if (comentarios!=null && comentarioProp!=null && comentarios.equals(comentarioProp)){
					ComentarioModificacion=comentarios;
				} 
			} 
			this.getSrvPublicacionService().modificaODECreado(identificador, user, titulo, ComentarioModificacion);

			//Aqui, al final del método volvemos a restaurar el manifest original
			//No se hace aqui, se hace posteriormente (despues del validar, puesto que, solo en ese caso "cambioManifest" es true)
//			if (cambioManifest && fichero_backup!=null && fichero!=null){
//				UtilesFicheros.copiar(fichero_backup, fichero);
...
//			}
	 ...
	 */
	@Override
	protected String handleAgregarManifiestoZIPByte(
			java.lang.String identificador,
			byte[] paqueteSCORM, String subId)
			throws java.lang.Exception {
...
//		{
			...
			String submanifest = rutaSubmanifest+"/imsmanifest.xml";
//		}
//		else
//		{
...
//		}
		...
		if (this.getZipDao().esZip(nuevoFichero.getPath())) {
			...
			this.getZipDao().descomprimir(nuevoFichero.getPath(), rutaSubmanifest);
			//TODO Si no es SCORM 2004, convertir
			es.pode.soporte.validador.TipoOde tipoOde = new es.pode.soporte.validador.TipoOde();
			tipoOde.obtenerTipoOde(rutaSubmanifest);
			String tipo=tipoOde.getTipo();
			...
			try {
				if (tipo !=null) {
					...
					if (tipo.equals(ConstantesAgrega.SCORM_12)) {
						xsl=utilidades.getProperty("xslt_scorm_12");
						if (logger.isDebugEnabled())
							logger.debug("EL ODE ES SCORM 12");

					}else if (tipo.equals(ConstantesAgrega.IMS_CP)) {
						xsl=utilidades.getProperty("xslt_ims_cp");
						if (logger.isDebugEnabled())
					...
					if (!xsl.equals("")) {
						...
						destTrans = rutaSubmanifest + "/imsmanifestTransformado.xml";
						transformador.transformar(xsl, submanifest, destTrans);
						// Falta renombrar el imsmanifestTransformado a imsmanifest pq asi está mal!!
						File fichOri = new File(submanifest);
						/*
						...
						if (logger.isDebugEnabled())logger.debug("HEMOS TRANSFORMADO DE " + submanifest + " " + destTrans);
//						como no era 2004, se ha hecho la transformacion y si era un rcp generamos el manifest
						...
						utilidades.borrarEsquemas(rutaSubmanifest, tipo);

						//NECESITAMOS LOS ESQUEMAS DE 2004
						utilidades.copiarEsquemas(rutaSubmanifest);
						if (logger.isDebugEnabled())logger.debug("HEMOS COPIADO LOS ESQUEMAS A " + rutaSubmanifest + "Y VAMOS A HACER LA VALIDACION LIGERA");
						//ahora tenemos que hacer una validacion ligera.. para quitar los no lom-es q puedan dar error de parseo
						ValidaVO validacionRCP = this.getSrvValidadorService().obtenervalidacionLigera(rutaSubmanifest,	ConstantesAgrega.RCP);
				...
				logger.debug("Borramos la carpeta "+rutaSubmanifest + "si existe."); //+"y tambien el zip que  esta en el temporal "+fileTemporal);
				UtilesFicheros.eliminar(destino);
				return; // new ResultadoOperacionVO(ERROR_EN_TRANSFORMACION, I18nModuloPublicacion.getPropertyValueI18n(ERROR_EN_TRANSFORMACION),idODE,new Long(0));
			}
...
//			}
	...
	protected void handleDesagregarSubmanifiestoRepositorio(
		...
		SrvLocalizadorService servicioLocalizador = this.getSrvLocalizadorService();
		LocalizadorVO localizador = servicioLocalizador
				.consultaLocalizador(identificador);
...
//		 Leo los datos del usuario. Compruebo configuracion para ver si
		...
		if (lomSubmanifiesto == null) {
			titulo = obtenerTituloProvisional(ODE_SIN_TITULO, new Locale(
		...
		try {
			...
			if ("0.0".equals(publica.getIdResultado())) {
				...
				Long vuelta=servicioLocalizador.calculaEspacioLocalizador(nuevoLocId.getIdentificador());
	...
	protected OdeVO handleEditarODE(String identificador) throws Exception {
		return handleEditarODEControlandoBackups(identificador,true);
	}
	...
	/**
	 * Comienza la edicion de un ODE existente en la carpeta taller del usuario.
	 * Esta operacion incluye cargar el Manifest en la cache y registrar los 
	 * objetos Lom asociados al Ode en el servicio de catalogación.
	 * Solo genererará un backup de los contenidos previos en caso que se le
	 * indique un true en el parametro hacerBackupODE.  
	 * El resultado es un objeto OdeVO con los datos del ODE a editar.
	 * 
	 * @param identificador
	 *            Identificador del Ode que se desea editar.
	 * @param hacerBackupODE
	 *            si se le pasa valor true se realizara un backup de los contenidos previos.
	 * @return Ode modificado
	 * @throws Exception
	 */
	@Override
	protected OdeVO handleEditarODEControlandoBackups(String identificador,
			Boolean hacerBackupODE) throws Exception {
		...
		if (obj instanceof es.pode.parseadorXML.castor.Manifest) {
			logger.warn("Se ha editado el ODE "
							+ ode
		...
		if(ruta.exists() && ruta.isFile())
		{
			...
			//manifest = this.getScormDao().parsearODEEager(ruta);
			if(DecisorOffline.esOffline() && manifest != null){
				//Para evitar problemas de discordancia entre identificadores, sobreescribimos el id del manifest con el id del ode
				manifest.setIdentifier(identificador);
				logger.info("IDENTIFICADOR DEL MANIFEST " + manifest.getIdentifier());
			}
			
			logger.info("voy a introducir el manifest en la cache");
			cacheEmpaquetacion.put(identificador, manifest);
			ode = mapearManifestOdeVO(identificador, manifest);
		}
		else
	...
	protected String handleEliminarSubmanifiesto(String identificador,
		...
		if (obj instanceof es.pode.parseadorXML.castor.Manifest) {
			logger.debug("El ode " + obj + " es de tipo manifest");
			manifest = (Manifest) obj;
			Manifest[] arrayManifest = manifest.getManifest();
			ArrayList<Manifest> tempList = new ArrayList<Manifest>();
			tempList.addAll(Arrays.asList(arrayManifest));

			for (int i = 0; i < tempList.size(); i++) {
				if ((tempList.get(i)).getIdentifier().equals(submanifiestoId)){
					//Submanifiesto a eliminar
					nombreSubmanifiestoEliminar = (tempList.get(i)).getBase();	
					tempList.remove(i);
				}
			}
			logger.debug("voy a insertar en el manifest el nuevo array de Submanifest sin el que quiero borrar");			
			manifest.setManifest(tempList.toArray(new Manifest[tempList.size()]));
...
//		org.setTitle(utilidades.getProperty("organizacion.por.defecto"));
		org.setTitle(ResourceBundle.getBundle("empaquetador", new Locale(idioma)).getString("organizacion.por.defecto"));
		orgs.addOrganization(org);
	...
	private Lom generarLomInicial(String titulo, String idioma, String identificador) throws Exception {
		...
		id.setCatalogo(this.getSrvPropiedadService().getValorPropiedad(AgregaProperties.CATALOGO_AGREGA));
	...
	protected OdeVO handleIniciarNuevoODE(String user, String titulo, String idioma) throws Exception {
		...
		odeVO.setLocalizadorURL(localizador.getPath());
		/*
...
//			 Elimino el lom de la hash de catalogacion
			...
			for (int i=0;i<arrayLomVO.length;i++){
				if (arrayLomVO[i]!=null && arrayLomVO[i].getLomHandler()!= null){
					...
					if(checkLom(lom)) {
						...
						if(logger.isDebugEnabled()) logger.debug("Recupero un Lom vacio de catalogacion. No se ha insertado en " + (href == null ? idElemento : idElemento + href));
					}
					} catch (Exception e) {
	...
	private ManifestAgrega comunCopiarMover(String destino, ManifestAgrega ma,
		...
		arrayItems = list.toArray(new Item[list.size()]);
	...
	protected DataHandler handleExportarRecursos(String identificador,
		...
		List<Resource> recur = new ArrayList<Resource>();
...
//		}
	 ...
	 */
	@Override
	protected AgregarVO handleImportarRecursos(String identificador,
			DataHandler paqueteRCP, String submanifestId,String usuario, String idioma) throws Exception {
		
		Manifest manifest = this.cacheEmpaquetacion.comprobarManifest(identificador);
		Manifest manifestImportado = null;
		AgregarVO agregarVO = new AgregarVO();
		...
		if (validaRCP(ficheroZip.getPath(), pathtempDestino)) {
			...
			//manifestImportado = this.getScormDao().parsearODEEager(rutaXml);

	   		guardarManifiestoControlandoBackups(identificador, false, false);
			ValidarLicenciasVO validarLicencias = getSrvGruposLicencias().cambiaLicenciaCompatibleRuta(localizador.getPath(), pathtempDestino, usuario, idioma);
			agregarVO.setResultado(validarLicencias.isResultado());
			agregarVO.setIdentificador(validarLicencias.getIdentificadorElemento());
			agregarVO.setLicenciaAdicional(validarLicencias.getLicenciaAdicional());
			agregarVO.setNecesitaCambio(validarLicencias.isNecesitaCambio());
						
			//Actualizamos el manifest en cache incluyendo asi los cambios en la licencia
			handleEditarODEControlandoBackups(identificador, false);
			
			//Actualizamos el objeto manifest con el contenido de la cache
			manifest = this.cacheEmpaquetacion.comprobarManifest(identificador);
			...
			for (int i = 0; i < recurs.length; i++) {
				if(recurs[i]!=null) {
					if(manifest.getResources()!=null) {
						...
						logger.debug("Manifest.resources es null!!");
					}
				} else {
					logger.debug("recurs["+i+"] es null");
				}
			}
			cacheEmpaquetacion.put(identificador, manifest);		
			logger.debug("se ha introducido el manifest en la cache");
			...
			if (!this.getZipDao().esZip(ficheroZip.getPath())) {
				...
				logger.debug("el fichero "+ficheroZip.getPath()+" no existe");
	...
	private boolean validaRCP(String zip, String pathTemp) throws Exception {
		...
		ValidaVO validacionVO = this.getSrvValidadorService().obtenervalidacionLigera(pathTemp, "RCP");
	...
	protected es.pode.empaquetador.negocio.servicio.ValidaVO handleValidar(
		...
		if ("BINARIA".equals(tipo)) {
			validaVO = this.getSrvValidadorService().obtenerValidacionBin(localizador.getPath());
		} else if ("LIGERA".equals(tipo)) {
			// Ligera: CA - Valida ODE
			// RCP - Valida Resource Content Package
			validaVO = this.getSrvValidadorService().obtenervalidacionLigera(localizador.getPath(), "CA");
		} else if ("COMPLETA".equals(tipo)) {
			validaVO = this.getSrvValidadorService().obtenerValidacion(localizador.getPath());
		} else {
			logger.error("Se ha llamado a handleValidar con tipo incorrecto: "+ tipo);
			throw new Exception("Se ha llamado a handleValidar con tipo incorrecto: "+ tipo);
		}

		es.pode.empaquetador.negocio.servicio.ValidaVO valida = (es.pode.empaquetador.negocio.servicio.ValidaVO) this
				.getBeanMapper().map(validaVO,es.pode.empaquetador.negocio.servicio.ValidaVO.class);

//Esto ya no se usa
//Provoca problemas por mal diseño. En su lugar ahora hay un metodo que se encarga de restaurar
//el backup creado previamente por el metodo guardarManifiesto
/*		
		String ruta = localizador.getPath() + BARRA + Utilidades.MANIFEST_NAME;
		File fichero = new File(ruta);
		String ruta_backup = localizador.getPath() + BARRA + SrvGestorManifestServiceImpl.manifest_backup;
		File fichero_backup= new File(ruta_backup);
		if (fichero_backup!=null && fichero!=null){
			UtilesFicheros.copiar(fichero_backup, fichero);
			UtilesFicheros.eliminar(fichero_backup);
			logger.info("Copiamos el fichero backup y lo eliminamos despues de validar");
		}
*/		
		return valida;
	}
	...
	protected TransicionVO[] handleObtenerObjetosCompartidos() throws Exception {
		...
		}else{//Si el usuario es parte del grupo todos hay que traer todos lo odes compartidos de la plataforma
			transiciones = this.getSrvPublicacionService().obtenODESCompartidos();
		}
		TransicionVO[] resultado = null;
		if (transiciones!=null && transiciones.length>0) {
			resultado = new TransicionVO[transiciones.length];
	...
	/*
	 * En el caso que exista el backup del manifiesto de un ODE lo restaura. Esto lo hace sobreescribiendo el manifiesto con el contenido del backup.
	 * Normalmente el backup del manifiesto lo crea el metodo guardarManifiesto.
	 *
	 * @param identificador: con el que se obtiene el localizador del ODE sobre el que se quiere operar.
	 * 
	 * @see es.pode.empaquetador.negocio.servicio.SrvGestorManifestServiceBase#handleRestaurarManifiesto(java.lang.String)
	 */
	@Override
	protected void handleRestaurarManifiesto(String identificador)
			throws Exception {
		...
		if (fichero_backup!=null && fichero!=null && fichero_backup.exists()){ 
			UtilesFicheros.copiar(fichero_backup, fichero);
			UtilesFicheros.eliminar(fichero_backup);
			logger.info("Copiamos el fichero backup y lo eliminamos despues de validar");
		}
