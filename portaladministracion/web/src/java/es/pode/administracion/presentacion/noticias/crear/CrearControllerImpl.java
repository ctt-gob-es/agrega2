...
import es.pode.administracion.presentacion.noticias.comun.ListaOrdenadaCategoria;
...
public class CrearControllerImpl extends CrearController {
	...
	public void obtenerCategoria(ActionMapping mapping, ObtenerCategoriaForm form, HttpServletRequest request, HttpServletResponse response) throws Exception 
	{
		...
		{	
			...
			CategoriaNoticiaTraducidaVO[] categorias = this.getSrvNoticiasService().obtenerCategoriasTraducidas(noticiasController.
					devuelveIdiomasTraducibles(idiomasPlataforma, idiomaLogado, idiomaPrioritario, idiomaSecundario));
			Collections.sort(Arrays.asList(categorias),new ListaOrdenadaCategoria());
			form.setCategoriaBackingList(Arrays.asList(categorias), "idCategoriaNoticia", "nombreCategoria");	
	...
	{		
		...
		ArrayList<String> idiomasList = new ArrayList<String>();
		...
		form.setIdiomasCategoriasBBDDAsArray(idiomasList.toArray(new String[0]));
...
//		Si hay nuevas traducciones de la categoria se inserta en la base de datos
		...
		ArrayList<String> nuevasCategorias = new ArrayList<String>();
		ArrayList<String> nuevasCategoriasIdiomas = new ArrayList<String>();
		...
		if (nuevasCategorias != null && nuevasCategorias.size() > 0)
		{
			try
			{
				...
				ArrayList<CategoriaIdiomaNoticiaVO> categoriasIdiomas = new ArrayList<CategoriaIdiomaNoticiaVO>();	
				...
				for (int j = 0; j < nuevasCategorias.size(); j++)
				{
					...
					categoriaNueva.setIdioma(nuevasCategoriasIdiomas.get(j).trim());					
					categoriaNueva.setNombreCategoria(nuevasCategorias.get(j).trim());
					categoriasIdiomas.add(categoriaNueva);
				}
				CategoriaIdiomaNoticiaVO[] categoriasIdiomasVector = categoriasIdiomas.toArray(new CategoriaIdiomaNoticiaVO[0]);
	...
	//		Se forma el vo de la noticia
			...
			ArrayList<DescripcionNoticiaVO> descripcionesNoticia = new ArrayList<DescripcionNoticiaVO>();	
	...
	//		Gallego
			...
			DescripcionNoticiaVO[] descripcionesNoticiaArray = descripcionesNoticia.toArray(new DescripcionNoticiaVO[0]);
...
//			Tratamiento de la imagen
	...
	public ArrayList<DescripcionNoticiaVO> devuelveListaDescripciones (ArrayList<DescripcionNoticiaVO> lista, String idioma, String titulo, String resumen, String cuerpo) throws Exception
	{			
...
//				Se comprueba el tamanho de resumen
		...
		return listaErrores.toArray(new String[0]);
	}
	...
	{	
		...
		ArrayList<String[]> listaErrores = dameListaErrores(errores);
		String[] erroresObligatorios = listaErrores.get(0);
		String[] erroresTamanho = listaErrores.get(1);		
...
//		Se analizan los errores obligatorios
		...
		if (erroresTamanho != null && erroresTamanho.length > 0)
		{
			...
			{
				//Hay mas de uno
				//TODO REVISAR!!! Se tira a la basura el StringBuilder!!
				StringBuilder errorBuilder = new StringBuilder();
				for (int k = 0; k < erroresTamanho.length - 1; k++)
					errorBuilder.append(RETORNOCARRO).append(traducirError(erroresTamanho[k], request)).append(PUNTO);
				error=error+Arrays.toString(errores);
	...
	{		
		...
		{
			String castellano = CrearControllerImpl.getResource(CASTELLANO, FICHERO, (Locale) request.getSession().getAttribute(ConstantesAgrega.DEFAULT_LOCALE));
			stReturn = "<h3 class=\"h3_espanol\">"+castellano+"</h3>";
		}		
		if (claveError.endsWith("ingles"))
		{
			String ingles = CrearControllerImpl.getResource(INGLES, FICHERO, (Locale) request.getSession().getAttribute(ConstantesAgrega.DEFAULT_LOCALE));
			stReturn = "<h3 class=\"h3_ingles\">"+ingles+"</h3>";
		}
		if (claveError.endsWith("catalan"))
		{
			String catalan = CrearControllerImpl.getResource(CATALAN, FICHERO, (Locale) request.getSession().getAttribute(ConstantesAgrega.DEFAULT_LOCALE));
			stReturn = "<h3 class=\"h3_catalan\">"+catalan+"</h3>";
		}
		if (claveError.endsWith("euskera"))
		{
			String euskera = CrearControllerImpl.getResource(EUSKERA, FICHERO, (Locale) request.getSession().getAttribute(ConstantesAgrega.DEFAULT_LOCALE));
			stReturn = "<h3 class=\"h3_vasco\">"+euskera+"</h3>";
		}
		if (claveError.endsWith("valenciano"))
		{
			String valenciano = CrearControllerImpl.getResource(VALENCIANO, FICHERO, (Locale) request.getSession().getAttribute(ConstantesAgrega.DEFAULT_LOCALE));
			stReturn = "<h3 class=\"h3_valenciano\">"+valenciano+"</h3>";
		}
		if (claveError.endsWith("gallego"))
		{
			String gallego = CrearControllerImpl.getResource(GALLEGO, FICHERO, (Locale) request.getSession().getAttribute(ConstantesAgrega.DEFAULT_LOCALE));
	...
	{
		return CrearControllerImpl.getResource(error, FICHERO, (Locale) request.getSession().getAttribute(ConstantesAgrega.DEFAULT_LOCALE));
	}
	 ...
	 * */
	public ArrayList<String[]> dameListaErrores (String[] errores) throws Exception
	{
		ArrayList<String[]> listaReturn = new ArrayList<String[]>();
		ArrayList<String> erroresObligatorios = new ArrayList<String>();
		ArrayList<String> erroresTamanho = new ArrayList<String>();
		...
		String[] erroresObligatoriosArray = erroresObligatorios.toArray(new String[0]);
		String[] erroresTamanhoArray = erroresTamanho.toArray(new String[0]);
...
//		Se comprueba si el formulario ha validado o no
	 ...
	 * */
	public ArrayList<String> dameIdiomas (String[] castellano,String[] catalan,String[] ingles,String[] euskera,String[] gallego,String[] valenciano)throws Exception
	{		
		ArrayList<String> idiomas = new ArrayList<String>();
	 ...
	 * */
	public ArrayList<String> dameTitulos (String[] castellano,String[] catalan,String[] ingles,String[] euskera,String[] gallego,String[] valenciano)throws Exception
	{
	 ...
	 * */
	public ArrayList<String> dameResumenes (String[] castellano,String[] catalan,String[] ingles,String[] euskera,String[] gallego,String[] valenciano)throws Exception
	{
	 ...
	 * */
	public ArrayList<String> dameCuerpos (String[] castellano,String[] catalan,String[] ingles,String[] euskera,String[] gallego,String[] valenciano)throws Exception
	{
	 ...
	 * */
	public ArrayList<String> dameCategorias (String[] castellano,String[] catalan,String[] ingles,String[] euskera,String[] gallego,String[] valenciano)throws Exception
	{
