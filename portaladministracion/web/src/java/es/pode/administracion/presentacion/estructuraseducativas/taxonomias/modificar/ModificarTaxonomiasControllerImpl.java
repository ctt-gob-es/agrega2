...
package es.pode.administracion.presentacion.estructuraseducativas.taxonomias.modificar;
...
/**
 * @see es.pode.administracion.presentacion.estructuraseducativas.taxonomias.modificar.ModificarTaxonomiasController
 */
public class ModificarTaxonomiasControllerImpl extends ModificarTaxonomiasController
{

	private static Logger logger= Logger.getLogger(ModificarTaxonomiasControllerImpl.class);
	...
	throws Exception 
    {
		...
		try{
			...
			logger.debug("localizadorIdiomas es "+Arrays.toString(localizadorIdioma));
			form.setIdiomaBuscadorBackingList(Arrays.asList(localizadorIdioma), "idLocalizacion", "nombre");
		}catch(Exception e)
		{
			logger.error("error en ModificarTaxonomiasControllerImpl.obtenerIdiomas" ,e );
		}
		
		try{
			form.setIdiomasIso(I18n.getInstance().obtenerIdiomasBuscables());
			String id= form.getIdentificadorVdex();
			VdexListaVO vdex= this.getSrvEstructurasEducativasService().listarFicherosTaxonomia(id);
			...
			logger.error("error al cargarlas Taxonomias a modificar " , e);
		}
     }
    ...
    public final void modificarTaxonomia(
    		ActionMapping mapping, 
    		ModificarTaxonomiaForm form, 
    		HttpServletRequest request, 
    		HttpServletResponse response) 
    throws Exception
    {
		...
		try{
			...
			if(TipoVdex.ARBOL.toString().equals(form.getTipo()))
			{
			...
			for (int i = 0; i < resultado.length; i++) 
			{
				...
				logger.error("error al dar de alta lista de taxonomias",e);
	...
	throws Exception 
	{
		...
		//1.- si el input esta vacío, no lo agrego a la lista de ficheros a subir
		//2.- si la lista de ficheros esta vacía, muestro el error de validación
		//3.- si hay ficheros, pero no coinciden con el idioma, muestro un mensaje de validación sin subir ningún fichero.
...
//    	VdexVO[] resultado=null;
		...
		{
			throw new ValidatorException("{estructuras.taxonomias.error.fichero.idioma.general}" );
		}
