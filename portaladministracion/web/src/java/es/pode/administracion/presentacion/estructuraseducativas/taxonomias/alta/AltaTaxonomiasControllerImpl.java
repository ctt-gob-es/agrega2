...
public class AltaTaxonomiasControllerImpl extends AltaTaxonomiasController
{
	private static Logger logger= Logger.getLogger(AltaTaxonomiasControllerImpl.class);

    public final void nuevaTaxonomia(
    		ActionMapping mapping, 
    		NuevaTaxonomiaForm form, 
    ...
    throws Exception
    {
		...
		}else if(errorIdioma.size()>0)
		{
			for (int i = 0; i < errorIdioma.size(); i++) {
				...
				this.saveErrorMessage(request, "estructuras.taxonomias.error.fichero.idioma" , lista );
			}
		}
		...
		try{
	    	...
	    	resultado = servicio.subirFicherosVdex(arrayParam.toArray(new ParamVdexVO[0]) , TipoVdex.TAXONOMIA);
    ...
    throws Exception
    {
		...
		{
			logger.error("error en AltaTaxonomiasControllerImpl.obtenerIdiomas");
		}
    }
