...
//			Introducimos las categorias idiomas
			...
			categoriaFaq.setNombre(listaCategorias[0]);		
...
//		Se ordena los array: Castellano, gallego, catalan, valenciano, euskera, ingles
		ArrayList listaOrdenada = controllerCategorias.devuelveArraysOrdenados(listaIdiomas.toArray(new String[0]), listaCategorias.toArray(new String[0]));
		listaCategorias = (ArrayList<String>)listaOrdenada.get(0);
		listaIdiomas = (ArrayList<String>)listaOrdenada.get(1);		
		
		if (validaForm(listaCategorias.toArray(new String[0]), listaIdiomas.toArray(new String[0]), request))
		{
	...
	private boolean validaForm (String[] categorias, String[] idiomas, HttpServletRequest request) throws Exception
    {
    	...
    	if (categorias == null || categorias.length == 0)
    	{
    		...
    		saveErrorMessage(request, CrearCategoriaNoticiaControllerImpl.ERRORCATEGORIAVACIA);
...
//	    		Se crea un array de string con el mismo tamaño que categorias hay. Cada posicion de este vector indicara una por una si la categoria es correcta
	    		...
	    		{
	    			if (categorias[i].trim().length() > CrearCategoriaNoticiaControllerImpl.MAXTAMCATEGORIA)
	    			{  
	    				...
	    				saveErrorMessage(request, CrearCategoriaNoticiaControllerImpl.ERRORCATEGORIAMAXTAMANHO, error);
//	    				Se indica que esta categoria no es correcta
	    			...
	    			else
	    			{
	    				...
	    				if (estaCategoriaDuplicada(categorias[i]))
	    				{
	    					...
	    					String[] error = new String[1];
		    				error[0] = controller.dameStringBandera(controller.devuelveClaveError(idiomas[i]), request); 				    				
		    				saveErrorMessage(request, CrearCategoriaNoticiaControllerImpl.ERRORCATEGORIADUPLICADA, error);
//		    				Se indica que esta categoria no es correcta
		    				validaArray[i] = "false";
	    				}
	    			}
...
//			La categoria que quiere crear se le elimina los acentos
			String categoriaACrearSinAcentos = CrearCategoriaNoticiaControllerImpl.removeAccents(categoriaACrear);	
...
//					Se obtiene la categoria que hay en base de datos sin acentos
					String categoriaSinAcentos = CrearCategoriaNoticiaControllerImpl.removeAccents(categoriasIdiomas[j].getNombre());	
