...
<%@ include file="/es/pode/administracion/presentacion/gestionCargaMasiva/listarCargaMasiva/formulario-listar-informes-carga-vars.jspf" %>
...
<!--		Inicio  PESTANIAS de FICHA		 -->
	<div id="ficha_pestanias">
		...
		<ul>
			<li><a href="<html:rewrite action="/ListarCargasEjecutadas/ListarCargasEjecutadas.do"/>"><bean:message key="cargas.pestania.listado"/></a></li>
			<li id="pest_activa"><a href="<html:rewrite action="/ListarInformesCarga/ListarInformesCarga.do"/>" id="seleccionada"><bean:message key="informes.pestania.listado" /></a></li>
		</ul>
	</div>
	...
	<fieldset>
	<form id="listarInformesCargaFormularioListarInformesCargaEliminarForm" action='<html:rewrite action="/ListarInformesCarga/FormularioListarInformesCargaEliminar"/>' method="post" >
		...
		<display:table name="${form.informes}" uid="fila" requestURI=""
				...
				<display:column sortable="true" sortProperty="nombre" style="valign:top;" class="tar" title="${informeValue}">
					<html:link action="/ListarInformesCarga/FormularioListarInformesCargaRecuperar?nombre=${fila.nombre}">
						${fila.nombre}
					</html:link>
				</display:column>
