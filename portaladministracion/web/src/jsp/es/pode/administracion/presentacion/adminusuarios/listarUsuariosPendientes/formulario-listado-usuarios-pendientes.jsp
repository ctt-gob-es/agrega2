...
<%@ include file="/es/pode/administracion/presentacion/adminusuarios/listarUsuariosPendientes/formulario-listado-usuarios-pendientes-vars.jspf" %>
...
<jsp:include page="/layout/messages.jsp" flush="true" />
	...
	<div id="ficha_pestanias">
		<div><h2><bean:message key="usuarios.cabecera.pendientes"/></h2></div>
		<ul>
			<li><a href="/${initParam.url_portaladministracion}/ListarUsuarios/ListarUsuarios.do" ><bean:message key="usuarios.pestania.listado"/></a></li>
			<li><a href="/${initParam.url_portaladministracion}/ListarUsuariosInactivos/ListarUsuariosInactivos.do"><bean:message key="usuarios.pestania.desactivados"/></a></li>
			<li id="pest_activa"><a href="/${initParam.url_portaladministracion}/ListarUsuariosPendientes/FormularioListadoUsuariosListarUsuariosPendientes.do" id="seleccionada"><bean:message key="usuarios.pestania.pendientes"/></a></li>
		</ul>
	</div>
	...
	<fieldset>
		<form id="listarUsuariosPendientesFormularioListadoUsuariosPendientesAltaForm" action='<html:rewrite action="/ListarUsuariosPendientes/FormularioListadoUsuariosPendientesBaja"/>' method="post" >
			...
			<display:table name="${form.usuarios}" uid="fila" requestURI=""
					...
					<display:column media="html" sortable="true" sortProperty="usuario" style="valign:top;" class="tar" title="${usuarioValue}">
						<html:link action="/ListarUsuariosPendientes/FormularioListadoUsuariosPendientesVerUsuarioPendiente?id=${fila.id}">
							${fila.usuario}
						</html:link>	
					</display:column>
					
					
					<!--		Columna de Dar de alta	(html)	-->
					
					<display:column media="html" style="valign:top;"> 
						<html:link action="/ListarUsuariosPendientes/FormularioListadoUsuariosPendientesAlta?id=${fila.id}">
							<bean:message key="usuariosPendientes.darAlta"/>
						</html:link>
			        </display:column>		
				    ...
				    <display:column media="excel pdf" title="${idiomaValueExport}" autolink="true" paramId="idioma">
							<bean:message key="crearTarea.${fila.idioma}"/>
					</display:column>
				    ...
				    <display:column media="excel pdf" title="${idiomaBusquedaValueExport}" autolink="true" paramId="idiomaBusqueda">
							<bean:message key="crearTarea.${fila.idiomaBusqueda}"/>
					</display:column>
			...
			<c:if test="${fn:length(form.usuarios) > 0}" >
				...
				<input class="boton_125_de_2 tipo"  type="submit"  value="<bean:message key='usuarios.eliminar'/>" />
